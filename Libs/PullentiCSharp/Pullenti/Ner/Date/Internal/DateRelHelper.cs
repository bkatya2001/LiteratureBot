        ...
        static List<Pullenti.Ner.ReferentToken> _createRefs(List<DateExToken.DateExItemToken> its)
        {
            ...
            for (int i = 0; i < its.Count; i++) 
            {
                ...
                if (it.Typ == DateExToken.DateExItemTokenType.Day) 
                {
                    ...
                    {
                        DateExToken.DateExItemToken it0 = its[i - 1];
                        int day = 0;
                        ...
                        else if (it0.Typ == DateExToken.DateExItemTokenType.Quartal && !it0.IsValueRelate) 
                        {
                            ...
                            Pullenti.Ner.Date.DateReferent dm = new Pullenti.Ner.Date.DateReferent();
                            dm.Month = m;
                            if (own != null) 
                                dm.Higher = own;
                            res.Add(new Pullenti.Ner.ReferentToken(dm, it.BeginToken, it.EndToken));
                            own = (d.Higher = dm);
                            if (((m == 1 || m == 3 || m == 5) || m == 7 || m == 8) || m == 10 || m == 12) 
                                day = 31;
                            else if (m == 2) 
                        ...
                        {
                            Pullenti.Ner.Date.DateReferent dy = new Pullenti.Ner.Date.DateReferent();
                            dy.Year = 99;
                            dy.IsRelative = true;
                            if (own != null) 
                                dy.Higher = own;
                            res.Add(new Pullenti.Ner.ReferentToken(dy, it.BeginToken, it.EndToken));
                            own = dy;
                            Pullenti.Ner.Date.DateReferent dm = new Pullenti.Ner.Date.DateReferent();
                            dm.Month = 12;
                            dm.Higher = own;
        ...
        static List<DateExToken.DateExItemToken> _createDateEx(Pullenti.Ner.Date.DateReferent dr)
        {
            ...
            for (; dr != null; dr = dr.Higher) 
            {
                ...
                foreach (Pullenti.Ner.Slot s in dr.Slots) 
                {
                    ...
                    else if (s.TypeName == Pullenti.Ner.Date.DateReferent.ATTR_SEASON) 
                    {

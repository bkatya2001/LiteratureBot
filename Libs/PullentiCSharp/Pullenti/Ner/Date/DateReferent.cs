...
namespace Pullenti.Ner.Date
{
    ...
    public class DateReferent : Pullenti.Ner.Referent
    {
        ...
        /// </summary>
        public const string ATTR_DECADE = "DECADE";
        /// <summary>
        /// Имя атрибута - год
        /// </summary>
        ...
        /// </summary>
        public const string ATTR_HALFYEAR = "HALFYEAR";
        /// <summary>
        ...
        /// </summary>
        public const string ATTR_SEASON = "SEASON";
        /// <summary>
        /// Имя атрибута - месяц
        /// </summary>
        ...
        /// </summary>
        public const string ATTR_NEWSTYLE = "NEWSTYLE";
        /// <summary>
        ...
        public int Season
        {
            get
            {
                if (this.FindSlot(ATTR_SEASON, null, true) == null && Higher != null) 
                    return Higher.Season;
                else 
        ...
        internal string _ToString(bool shortVariant, Pullenti.Morph.MorphLang lang, int lev, int fromRange)
        {
            ...
            if (IsRelative) 
            {
                ...
                foreach (Pullenti.Ner.Slot s in Slots) 
                {
                    ...
                    else if (s.TypeName == ATTR_DECADE) 
                    {
                        word = "декада";
                        int.TryParse(s.Value as string, out val);
                    }
                    else if (s.TypeName == ATTR_HALFYEAR) 
                    {
                        word = "полугодие";
                        int.TryParse(s.Value as string, out val);
                    }
                    else if (s.TypeName == ATTR_SEASON) 
                    {
                        int.TryParse(s.Value as string, out val);
                        if (val == 1) 
            ...
            if (y == 0 && ten != 0) 
                res.AppendFormat("{0} {1}", ten, (lang.IsEn ? "decade" : "декада"));
            if (d > 0) 
                res.Append(d);
            if (m > 0 && m <= 12) 
            {
            ...
            {
                DateReferent dt = this.GetSlotValue(ATTR_NEWSTYLE) as DateReferent;
                if (dt != null) 
                    res.AppendFormat(" (новый стиль: {0})", dt.ToString(shortVariant, lang, lev + 1));
            }
        ...
        public static int Compare(DateReferent d1, DateReferent d2)
        {
            ...
            {
                DatePointerType p1 = d1.Pointer;
                DatePointerType p2 = d2.Pointer;

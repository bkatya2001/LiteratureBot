...
namespace Pullenti.Ner
{
    ...
    public class Referent
    {
        ...
        public void Deserialize(Stream stream, List<Referent> all, SourceOfAnalysis sofa, bool newFormat = false)
        {
            ...
            for (int i = 0; i < cou; i++) 
            {
                ...
                if (newFormat) 
                {
                    byte bb = (byte)stream.ReadByte();
                    if (bb == 2) 
                    {
                        int id1 = Pullenti.Ner.Core.Internal.SerializerHelper.DeserializeInt(stream);
                        if (id1 > 0 && id1 <= all.Count) 
                            val = all[id1 - 1];
                    }
                    else if (bb == 1) 

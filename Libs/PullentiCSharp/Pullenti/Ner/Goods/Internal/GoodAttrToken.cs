...
namespace Pullenti.Ner.Goods.Internal
{
    public class GoodAttrToken : Pullenti.Ner.MetaToken
    {
        ...
        static List<GoodAttrToken> _tryParseList(Pullenti.Ner.Token t)
        {
            ...
            for (Pullenti.Ner.Token tt = t; tt != null; tt = tt.Next) 
            {
                ...
                if ((tt is Pullenti.Ner.TextToken) && nextSeq) 
                {
                    ...
                    if (dc.Equals(Pullenti.Morph.MorphClass.Verb)) 
                    {
                        if (!_isSpecVerb(tt)) 
                            break;
                    }
        ...
        static GoodAttrToken _tryParse_(Pullenti.Ner.Token t, GoodAttrToken key, bool canBeMeasure, bool isChars)
        {
            ...
            if ((((tok = m_StdAbbrs.TryParse(t, Pullenti.Ner.Core.TerminParseAttr.No)))) != null) 
            {
                ...
                if (ty == Pullenti.Ner.Goods.GoodAttrType.Referent) 
                {
                    ...
                    {
                        Pullenti.Ner.ReferentToken rt = tt.Kit.ProcessReferent("ORGANIZATION", tt, null);
                        if (rt != null) 
                    ...
                    {
                        Pullenti.Ner.ReferentToken rt = tt.Kit.ProcessReferent("ORGANIZATION", tt.Next, null);
                        if (rt != null) 
                        {
            ...
            if (key == null && !isChars) 
            {
                ...
                if ((((!t.Chars.IsAllUpper || isAllUpper)) && ((t.Morph.Class.IsNoun || t.Morph.Class.IsUndefined)) && t.Chars.IsCyrillicLetter) && (t is Pullenti.Ner.TextToken)) 
                {
                    ...
                    if (ok) 
                    {
                        ...
                        if ((t.Next != null && t.Next.IsHiphen && (t.Next.Next is Pullenti.Ner.TextToken)) && ((t.Next.Next.Chars.IsAllLower || t.Next.Next.Chars.Equals(t.Chars)))) 
                        {
            ...
            if (t.Chars.IsCyrillicLetter) 
            {
                ...
                {
                    if (((t.GetMorphClassInDictionary().IsAdjective || t.Morph.Class.Equals(Pullenti.Morph.MorphClass.Adjective))) && pref == null) 
        ...
        static GoodAttrToken _tryParseNum(Pullenti.Ner.Token t)
        {
            ...
            List<Pullenti.Ner.Measure.Internal.NumbersWithUnitToken> mts = Pullenti.Ner.Measure.Internal.NumbersWithUnitToken.TryParseMulti(t, null, Pullenti.Ner.Measure.Internal.NumberWithUnitParseAttr.No);

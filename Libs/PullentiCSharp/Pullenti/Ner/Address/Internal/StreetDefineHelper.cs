    ...
    public static class StreetDefineHelper
    {
        ...
        internal static AddressItemToken TryParseStreet(List<StreetItemToken> sli, bool extOntoRegim = false, bool forMetro = false, bool streetBefore = false)
        {
            ...
            for (i = 0; i < sli.Count; i++) 
            {
                ...
                else if (sli[i].Typ == StreetItemType.Noun) 
                {
                    ...
                    {
                        if (sli[i + 1].Typ == StreetItemType.Noun && StreetItemToken._isRegion(sli[i + 1].Termin.CanonicText)) 
                        {
                            sli[i + 1].BeginToken = sli[i].BeginToken;
                            sli.RemoveAt(i);
                        }
                        else if ((((i + 2) < sli.Count) && sli[i + 1].Typ == StreetItemType.Number && sli[i + 2].Typ == StreetItemType.Noun) && StreetItemToken._isRegion(sli[i + 2].Termin.CanonicText)) 
                        {
                            sli[i + 1].BeginToken = sli[i].BeginToken;
                            sli.RemoveAt(i);
                            i--;
        ...
        static AddressItemToken TryDetectNonNoun(List<StreetItemToken> sli, bool ontoRegim, bool forMetro, bool streetBefore)
        {
            ...
            else 
            {
                ...
                if (ait.Typ == AddressItemType.House && ait.Value != null) 
                    ...
                    return null;
                else if (ait.Typ == AddressItemType.Number && (ait.BeginToken.WhitespacesBeforeCount < 2)) 
                {
        ...
        static AddressItemToken _tryParseFix(List<StreetItemToken> sits)
        {
            ...
            if (sits[0].IsRailway) 
            {
                ...
                str.AddSlot(Pullenti.Ner.Address.StreetReferent.ATTR_TYP, "железная дорога", false, 0);

...
namespace Pullenti.Ner.Uri
{
    ...
    public class UriAnalyzer : Pullenti.Ner.Analyzer
    {
            ...
            for (Pullenti.Ner.Token t = kit.FirstToken; t != null; t = t.Next) 
            {
                ...
                if (tok != null) 
                {
                    ...
                    if (i == 7) 
                    {
                        ...
                        if (t0 == null) 
                            continue;
                        Pullenti.Ner.ReferentToken rt = _TryAttachKadastr(t0);
                        if (rt == null) 
                            continue;
                ...
                if (((((t is Pullenti.Ner.NumberToken) && (t as Pullenti.Ner.NumberToken).Typ == Pullenti.Ner.NumberSpellingType.Digit && (t.LengthChar < 3)) && t.Next != null && t.Next.IsChar(':')) && !t.IsWhitespaceAfter && !t.Next.IsWhitespaceAfter) && (t.Next.Next is Pullenti.Ner.NumberToken)) 
                {
                    Pullenti.Ner.ReferentToken rt = _TryAttachKadastr(t);
                    if (rt != null) 
                    {
                        rt.Referent = ad.RegisterReferent(rt.Referent);
                        kit.EmbedToken(rt);
                ...
                if (t.IsValue("КАДАСТРОВЫЙ", null)) 
                {
                    ...
                    if (t1 != null) 
                    {
                        if (t1.IsHiphen || t1.IsTableControlChar) 
                            t1 = t1.Next;
                        Pullenti.Ner.ReferentToken rt = _TryAttachKadastr(t1);
                        if (rt != null) 
                        {
                            rt.Referent = ad.RegisterReferent(rt.Referent);
                            rt.BeginToken = t;
        ...
        static Pullenti.Ner.ReferentToken _TryAttachKadastr(Pullenti.Ner.Token t)
        {
            if (!(t is Pullenti.Ner.NumberToken) || t.LengthChar > 2) 
                return null;
            ...
            Pullenti.Ner.ReferentToken rt = new Pullenti.Ner.ReferentToken(null, t, t);
            for (; t != null; t = t.Next) 
            {
                Pullenti.Ner.NumberToken num = t as Pullenti.Ner.NumberToken;
                if (num == null || num.Typ != Pullenti.Ner.NumberSpellingType.Digit || num.IntValue == null) 
                    break;
                vals.Add(num.Value);
                rt.EndToken = t;
                if ((t.Next != null && t.Next.IsChar(':') && (t.Next.Next is Pullenti.Ner.NumberToken)) && !t.IsWhitespaceAfter && !t.Next.IsWhitespaceAfter) 
                {
                    t = t.Next;
                    continue;
                }
            ...
            if (vals.Count != 4) 
                return null;
            UriReferent uri = new UriReferent();
            uri.Value = string.Format("{0}:{1}:{2}:{3}", vals[0], vals[1], vals[2], vals[3]);
            uri.Scheme = "КАДАСТР";
            rt.Referent = uri;
            return rt;
        }
        ...
        public static void Initialize()
        {
            ...
            try 
            {
                ...
                m_Schemes.Add(t);
                t = new Pullenti.Ner.Core.Termin("КАДАСТРОВЫЙ НОМЕР") { Tag = 7 };

    ...
    public class FragToken : Pullenti.Ner.MetaToken
    {
        ...
        static FragToken CreateProjectTitle(Pullenti.Ner.Token t0, Pullenti.Ner.Instrument.InstrumentReferent doc)
        {
            ...
            for (t = t00; t != null; t = t.Next) 
            {
                ...
                {
                    if (t.Next.GetMorphClassInDictionary().Equals(Pullenti.Morph.MorphClass.Verb)) 
                    {
                        t11 = t;
                        break;
                    }
        ...
        static Pullenti.Ner.ReferentToken _createJustParticipant(Pullenti.Ner.Token t, string typ)
        {
            ...
            for (; t != null; t = t.Next) 
            {
                ...
                Pullenti.Ner.Org.Internal.OrgItemTypeToken ty = Pullenti.Ner.Org.Internal.OrgItemTypeToken.TryAttach(t, true);
                if (ty != null) 
                {
                    t1 = (t = ty.EndToken);
                    ok = true;
                ...
                {
                    if (t.Morph.Class.Equals(Pullenti.Morph.MorphClass.Verb) || t.Morph.Class.Equals(Pullenti.Morph.MorphClass.Adverb)) 
                        break;
                }
                ...
                if (dt.Typ == Pullenti.Ner.Decree.Internal.DecreeToken.ItemType.Org || dt.Typ == Pullenti.Ner.Decree.Internal.DecreeToken.ItemType.Owner) 
                {
                    ...
                    for (Pullenti.Ner.Token ttt = dt.EndToken.Next; ttt != null; ttt = ttt.Next) 
                    {
                        if (ttt.WhitespacesBeforeCount > 15) 
                            break;
                        if (ttt.GetMorphClassInDictionary().Equals(Pullenti.Morph.MorphClass.Verb)) 
                        {
                            dt = null;
                            break;
                        }
                        ...
                        else if (!ttt.Chars.Equals(dt.BeginToken.Chars) && ttt.IsNewlineBefore) 
                            break;
                        else 
                            dt.EndToken = ttt;
                    }
        ...
        public static FragToken CreateDocument(Pullenti.Ner.Token t, int maxChar, Pullenti.Ner.Instrument.InstrumentKind rootKind = Pullenti.Ner.Instrument.InstrumentKind.Undefined)
        {
            ...
            for (i = 0; i < li.Count; i++) 
            {
                if (li[i].Typ == ILTypes.Appendix || li[i].Typ == ILTypes.Approved) 
                {
                    if (li[i].Typ == ILTypes.Approved) 
                    {
                        ...
                        if (i == 0) 
                            continue;
                        if (!li[i].IsNewlineAfter) 
                        {
                            if (((i + 1) < li.Count) && li[i + 1].IsNewlineAfter && li[i + 1].Typ == ILTypes.Undefined) 
                            {
                            }
                            else 
                                continue;
            ...
            if (maxChar == 0 && i1 == li.Count) 
            {
                for (i = 0; i < li.Count; i++) 
                {
                    if (li[i].Typ == ILTypes.Person && li[i].IsNewlineBefore && ((!li[i].HasTableChars || li[i].IsPurePerson))) 
                    {
                        int j;
                        bool dat = false;
                        bool num = false;
        ...
        public static FragToken _createEditions(Pullenti.Ner.Token t)
        {
            ...
            if ((t.IsNewlineBefore && t.IsValue("С", null) && t.Next != null) && t.Next.IsValue("ИЗМЕНЕНИЕ", null)) 
            {
                ...
                for (t = t.Next.Next; t != null; t = t.Next) 
                {
                    ...
                    {
                        Pullenti.Ner.Date.Internal.DateItemToken dd = Pullenti.Ner.Date.Internal.DateItemToken.TryParse(t, null, false);
                        if (dd != null) 
        ...
        static FragToken _createOwner(Pullenti.Ner.Token t)
        {
            ...
            for (Pullenti.Ner.Token tt = t; tt != null; tt = tt.Next) 
            {
                ...
                if ((r is Pullenti.Ner.Geo.GeoReferent) && tt.IsNewlineBefore) 
                    break;
                t1 = tt;
                Pullenti.Ner.ReferentToken oo = tt.Kit.ProcessReferent("ORGANIZATION", tt, null);
                if (oo != null) 
                {
                    t1 = (tt = oo.EndToken);
                    continue;
                ...
                if (npt != null) 
                {
                    if (tt == t) 
                    {
                        Pullenti.Ner.Org.Internal.OrgItemTypeToken typ = Pullenti.Ner.Org.Internal.OrgItemTypeToken.TryAttach(tt, false);
                        if (typ != null) 
                        {
                            keyword = true;
                            t1 = (tt = typ.EndToken);

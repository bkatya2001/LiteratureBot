...
namespace Pullenti.Ner.Core
{
    ...
    public static class NumberHelper
    {
        ...
        public static Pullenti.Ner.NumberToken TryParseAge(Pullenti.Ner.Token t)
        {
            ...
            if (i > 3) 
            {
                Termin term = m_Nums.Find(s.Substring(0, i));
                if (term != null) 
                    return new Pullenti.Ner.NumberToken(tt, tt, term.Tag.ToString(), Pullenti.Ner.NumberSpellingType.Age) { Morph = tt.Morph };
            }
            return null;
        ...
        static Pullenti.Ner.MetaToken AnalizeNumberTail(Pullenti.Ner.TextToken tt, string val)
        {
            ...
            foreach (Pullenti.Morph.MorphWordForm v in vars) 
            {
                if (v.Class.IsAdjective && Pullenti.Morph.LanguageHelper.EndsWith(v.NormalCase, s) && v.Number != Pullenti.Morph.MorphNumber.Undefined) 
                {
                    ...
                    {
                        if (it.Class.Equals(v.Class) && it.Number == v.Number && ((it.Gender == v.Gender || v.Number == Pullenti.Morph.MorphNumber.Plural))) 
                        {
                            it.Case |= v.Case;
                            ok = true;
                            break;
        ...
        public static Pullenti.Ner.NumberToken TryParseRealNumber(Pullenti.Ner.Token t, bool canBeInteger = false, bool noWhitespace = false)
        {
            ...
            if (t.Next != null && t.Next.IsValue("ЦЕЛЫЙ", "ЦІЛИЙ")) 
            {
                ...
                if (tt1 != null && tt1.IsCommaAnd) 
                    tt1 = tt1.Next;
                if ((tt1 is Pullenti.Ner.NumberToken) || (((tt1 is Pullenti.Ner.TextToken) && tt1.IsValue("НОЛЬ", "НУЛЬ")))) 
                {
        ...
        public static bool CheckPureNumber(Pullenti.Ner.Token t)
        {
            if (t is Pullenti.Ner.NumberToken) 
                return CheckPureNumber((t as Pullenti.Ner.NumberToken).EndToken);
            if (!(t is Pullenti.Ner.TextToken)) 
                return false;
            foreach (string w in m_1Words) 
            {
                if (t.IsValue(w, null)) 
                    return true;
            }
            foreach (string w in m_10Words) 
            {
                if (t.IsValue(w, null)) 
                    return true;
            }
            foreach (string w in m_100Words) 
            {
                if (t.IsValue(w, null)) 
                    return true;
            }

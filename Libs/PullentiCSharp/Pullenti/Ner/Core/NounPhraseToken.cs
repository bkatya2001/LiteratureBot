...
namespace Pullenti.Ner.Core
{
    ...
    public class NounPhraseToken : Pullenti.Ner.MetaToken
    {
        ...
        {
            NounPhraseToken res = new NounPhraseToken(BeginToken, EndToken);
            res.Morph = Morph.Clone();
            res.Chars = Chars;
            if (Noun is Pullenti.Ner.Core.Internal.NounPhraseItem) 
                res.Noun = (Noun as Pullenti.Ner.Core.Internal.NounPhraseItem).Clone();
            else 
                res.Noun = Noun;
            res.InternalNoun = InternalNoun;
            if (Adverbs != null) 
                res.Adverbs = new List<Pullenti.Ner.TextToken>(Adverbs);
            foreach (Pullenti.Ner.MetaToken a in Adjectives) 
            {
                if (a is Pullenti.Ner.Core.Internal.NounPhraseItem) 
                    res.Adjectives.Add((a as Pullenti.Ner.Core.Internal.NounPhraseItem).Clone());
                else 

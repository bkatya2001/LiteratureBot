    ...
    public static class BracketHelper
    {
        ...
        public static BracketSequenceToken TryParse(Pullenti.Ner.Token t, BracketParseAttr attrs = BracketParseAttr.No, int maxTokens = 100)
        {
            ...
            for (t = t0.Next; t != null; t = t.Next) 
            {
                            ...
                            if (tt.IsChar('.')) 
                            {
                                for (tt = tt.Next; tt != null; tt = tt.Next) 
                                {
                                    ...
                                    else if (tt.IsCharOf(m_OpenChars) || tt.IsCharOf(m_CloseChars)) 
                                    {
                                        ...
                                        if (BracketHelper.CanBeEndOfSequence(tt, false, null, false) && CanBeCloseChar(bb2.Char, brList[0].Char, false)) 
                                            ok = true;
                                        break;
                                    }
                                }
                ...
                else 
                {
                    ...
                    if (((attrs & BracketParseAttr.CanContainsVerbs)) == BracketParseAttr.No) 
                    {
                        ...
                        {
                            if (t.GetMorphClassInDictionary().Equals(Pullenti.Morph.MorphClass.Verb)) 
                            {
                        ...
                        {
                            if (t.Morph.Class.Equals(Pullenti.Morph.MorphClass.Verb) && t.Chars.IsAllLower) 
                                break;
                        }
            ...
            while (brList.Count > 3) 
            {
                ...
                if ((brList[i].CanBeClose && brList[i - 1].CanBeOpen && !CanBeCloseChar(brList[i].Char, brList[0].Char, false)) && CanBeCloseChar(brList[i].Char, brList[i - 1].Char, false)) 
                {
            ...
            while (brList.Count >= 4) 
            {
                ...
                for (int i = 1; i < (brList.Count - 2); i++) 
                {
                    if ((brList[i].CanBeOpen && !brList[i].CanBeClose && brList[i + 1].CanBeClose) && !brList[i + 1].CanBeOpen) 
                    {
                        ...
                        {
                            ok = true;
                            if ((i == 1 && ((i + 2) < brList.Count) && brList[i + 2].Char == ')') && brList[i + 1].Char != ')' && CanBeCloseChar(brList[i + 1].Char, brList[i - 1].Char, false)) 
            ...
            {
                if (CanBeCloseChar(brList[3].Char, brList[0].Char, false)) 
                {
            ...
            if ((res == null && brList.Count >= 3 && brList[2].CanBeClose) && !brList[2].CanBeOpen) 
            {
                ...
                {
                    if (CanBeCloseChar(brList[1].Char, brList[0].Char, false)) 
                        return new BracketSequenceToken(brList[0].Source, brList[1].Source);
                }
                bool ok = true;
                if (CanBeCloseChar(brList[2].Char, brList[0].Char, false) && CanBeCloseChar(brList[1].Char, brList[0].Char, false) && brList[1].CanBeClose) 
                {
                    ...
                    for (Pullenti.Ner.Token tt = brList[0].Source.Previous; tt != null; tt = tt.Previous) 
                    {
                        ...
                        if (CanBeCloseChar(ch, brList[0].Char, false)) 
                            lev1++;
                        else if (CanBeCloseChar(brList[1].Char, ch, false)) 
                        {
                            lev1--;
                            if (lev1 < 0) 
                                return new BracketSequenceToken(brList[0].Source, brList[1].Source);
                ...
                if (ok && CanBeCloseChar(brList[2].Char, brList[0].Char, false)) 
                {
                ...
                else if (ok && CanBeCloseChar(brList[2].Char, brList[1].Char, false) && brList[0].CanBeOpen) 
                {
                    if (CanBeCloseChar(brList[2].Char, brList[0].Char, false)) 
                    {
                        BracketSequenceToken intern = new BracketSequenceToken(brList[1].Source, brList[2].Source);
                        res = new BracketSequenceToken(brList[0].Source, brList[2].Source);
                        res.Internal.Add(intern);
            ...
            if (res == null && brList.Count > 1) 
            {
                bool hasCr = false;
                for (Pullenti.Ner.Token tt = brList[0].Source.Next; tt.EndChar < brList[1].Source.BeginChar; tt = tt.Next) 
                {
                    if (tt.IsNewlineAfter) 
                        hasCr = true;
                }
                if (CanBeCloseChar(brList[1].Char, brList[0].Char, hasCr)) 
                ...
                for (Pullenti.Ner.Token tt = t0.Next; tt != null; tt = tt.Next,cou++) 
                {
                    ...
                    if (mt.EndToken is Pullenti.Ner.TextToken) 
                    {
                        if ((mt.EndToken as Pullenti.Ner.TextToken).IsCharOf(m_CloseChars)) 
                        {
                            ...
                            if (bb.CanBeClose && CanBeCloseChar(bb.Char, brList[0].Char, false)) 
        ...
        static bool CanBeCloseChar(char close, char open, bool strict = false)
        {

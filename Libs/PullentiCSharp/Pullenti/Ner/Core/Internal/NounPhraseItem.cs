...
namespace Pullenti.Ner.Core.Internal
{
    ...
    class NounPhraseItem : Pullenti.Ner.MetaToken
    {
        ...
        {
            NounPhraseItem res = new NounPhraseItem(BeginToken, EndToken);
            res.Morph = Morph.Clone();
            ...
            res.AdjMorph.AddRange(AdjMorph);
            res.NounMorph.AddRange(NounMorph);
            return res;
        }
        ...
        public override string GetNormalCaseText(Pullenti.Morph.MorphClass mc = null, Pullenti.Morph.MorphNumber num = Pullenti.Morph.MorphNumber.Undefined, Pullenti.Morph.MorphGender gender = Pullenti.Morph.MorphGender.Undefined, bool keepChars = false)
        {
            ...
            foreach (Pullenti.Morph.MorphBaseInfo it in Morph.Items) 
            {
                ...
                if (BeginToken != EndToken && EndToken.IsValue(res1, null)) 
                    res1 = Pullenti.Ner.Core.MiscHelper.GetTextValue(BeginToken, EndToken.Previous, Pullenti.Ner.Core.GetTextAttr.No) + res1;
            ...
            if (t is Pullenti.Ner.TextToken) 
            {
                    ...
                    foreach (Pullenti.Morph.MorphBaseInfo wf in t.Morph.Items) 
                    {
                        if ((wf is Pullenti.Morph.MorphWordForm) && (wf as Pullenti.Morph.MorphWordForm).IsInDictionary) 
                        {
                            ...
                            if (wf.Class.IsAdjective) 
                            {
                                ...
                                {
                                    if (t.GetMorphClassInDictionary().Equals(Pullenti.Morph.MorphClass.Adjective) && !t.Morph.ContainsAttr("неизм.", null)) 
                                    {
                                    }
                                    else 
                                        _isDoubtAdj = true;
                ...
                if (mc0.IsProperName && !t.Chars.IsAllLower) 
                {
                    foreach (Pullenti.Morph.MorphBaseInfo wff in t.Morph.Items) 
                    {
                        ...
                        if (wf == null) 
                            continue;
                        if (wf.NormalCase == "ГОР" || wf.NormalCase == "ГОРЫ") 
                            continue;
                ...
                if (mc1.Equals(Pullenti.Morph.MorphClass.Verb) && t.Morph.Case.IsUndefined) 
                    return null;
                ...
                foreach (Pullenti.Morph.MorphBaseInfo v in t.Morph.Items) 
                {
                    ...
                    if (canBeNoun) 
                    {
                        ...
                        if (!added) 
                        {
                            ...
                            else if (((attrs & Pullenti.Ner.Core.NounPhraseParseAttr.ParseNumericAsAdjective)) != Pullenti.Ner.Core.NounPhraseParseAttr.No && items.Count > 0 && (((items[0].BeginToken is Pullenti.Ner.NumberToken) || (items[items.Count - 1].BeginToken is Pullenti.Ner.NumberToken)))) 
                            {
                                it.NounMorph.Add(new NounPhraseItemTextVar(v, t));
                                it.CanBeNoun = true;
                            }

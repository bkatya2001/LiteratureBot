...
namespace Pullenti.Ner.Core
{
    ...
    public class AnalysisKit
    {
        ...
        public Pullenti.Ner.Token CorrectWordByMorph(Pullenti.Ner.Token tt, Pullenti.Morph.MorphLang lang)
        {
        ...
        void CorrectWordsByMorph(Pullenti.Morph.MorphLang lang)
        {
            ...
            {
                Pullenti.Ner.Token t = this.CorrectWordByMorph(tt, lang);
                if (t != null) 
                    tt = t;
            }
        }
        ...
        /// <param name="param">параметр, поддерживаемый анализатором</param>
        ...
        public Pullenti.Ner.ReferentToken ProcessReferent(string analyzerName, Pullenti.Ner.Token t, string param = null)
        {
            if (Processor == null) 
                return null;
            ...
            Pullenti.Ner.ReferentToken res = a.ProcessReferent(t, param);
        ...
        public void FixAnalyzer(string analyzerName, bool ban)
        {
            if (ban) 
                m_AnalyzerStack.Add(analyzerName);
        ...
        public Pullenti.Ner.Referent CreateReferent(string typeName)
        {
            ...
            return Pullenti.Ner.ProcessorService.CreateReferent(typeName);
        }
        ...
        public void Serialize(Stream stream, bool newFormat = false)
        {
            stream.WriteByte((byte)0xAA);
            stream.WriteByte((byte)((newFormat ? 2 : 1)));
        ...
        public bool Deserialize(Stream stream)
        {
            ...
            for (int i = 0; i < cou; i++) 
            {
                m_Entities[i].Deserialize(stream, m_Entities, m_Sofa, ((int)b) > 1);
            }

...
namespace Pullenti.Ner.Core
{
    ...
    public static class MiscHelper
    {
        ...
        static string restChars(Pullenti.Ner.TextToken t, Pullenti.Ner.Referent r)
        {
            ...
            if (mc.IsAdjective) 
                return (canCapUp ? MiscHelper.ConvertFirstCharUpperAndOtherLower(t.Term) : t.Term.ToLower());
            if (mc.Equals(Pullenti.Morph.MorphClass.Noun)) 
                return (canCapUp ? MiscHelper.ConvertFirstCharUpperAndOtherLower(t.Term) : t.Term.ToLower());
            return t.Term;
        }
        /// <summary>
        ...
        /// <param name="useMorphSample">использовать именную группу примера для морфологии</param>
        ...
        {
            return GetTextMorphVariant(txt, cas, (pluralNumber ? Pullenti.Morph.MorphNumber.Plural : Pullenti.Morph.MorphNumber.Undefined));
        }
        /// <summary>
        ...
        /// <param name="num">число (по умолчанию, MorphNumber.Undefined - оставить как есть)</param>
        /// <return>результат (в крайнем случае, вернёт исходную строку, если ничего не получилось)</return>
        public static string GetTextMorphVariant(string txt, Pullenti.Morph.MorphCase cas, Pullenti.Morph.MorphNumber num = Pullenti.Morph.MorphNumber.Undefined)
        {
            Pullenti.Ner.AnalysisResult ar = Pullenti.Ner.ProcessorService.EmptyProcessor.Process(new Pullenti.Ner.SourceOfAnalysis(txt), null, null);
            if (ar == null || ar.FirstToken == null) 
                return txt;
            ...
            if ((t0 is Pullenti.Ner.TextToken) && !t0.Chars.IsLetter) 
            {
            ...
            if (npt != null) 
            {
                ...
                {
                    if (npt.Morph.Number == Pullenti.Morph.MorphNumber.Plural || npt.Morph.Number == Pullenti.Morph.MorphNumber.Singular) 
                        num = npt.Morph.Number;
                }
                if (cas.IsAccusative && num == Pullenti.Morph.MorphNumber.Plural) 
                {
                    accCorr = true;
                ...
                for (Pullenti.Ner.Token t = npt.BeginToken; t != null && t.EndChar <= npt.EndChar; t = t.Next) 
                {
                    ...
                    else if (t is Pullenti.Ner.TextToken) 
                    {
                        foreach (Pullenti.Morph.MorphBaseInfo it in t.Morph.Items) 
                        {
                            ...
                            if (num == Pullenti.Morph.MorphNumber.Plural && wf.Number == Pullenti.Morph.MorphNumber.Singular && t != npt.Anafor) 
                                continue;
                            if (num == Pullenti.Morph.MorphNumber.Singular && wf.Number == Pullenti.Morph.MorphNumber.Plural && t != npt.Anafor) 
                                continue;
                            if (isNoun) 
                            {

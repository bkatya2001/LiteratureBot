                    ...
                    if (noun != null) 
                    {
                        if (k == 1) 
                        {
                            if (noun.TerminItem.CanonicText == "РАЙОН" || noun.TerminItem.CanonicText == "ОБЛАСТЬ" || noun.TerminItem.CanonicText == "СОЮЗ") 
                            {
                                ...
                                if (!ok) 
                                    ok = MiscLocationHelper.CheckGeoObjectAfter(li[1].EndToken, false);
                                if (!ok) 
                                {
            ...
            if (exObj != null) 
            {
                ...
                if (exObj.IsDoubt && noun == null) 
                {
                    ...
                    if (!ok2) 
                    {
                        ...
                        else if (MiscLocationHelper.CheckGeoObjectBefore(exObj.BeginToken.Previous, false)) 
                        {
                        }
                        else 
                            return null;
                    ...
                    if (li.IndexOf(noun) < li.IndexOf(newName)) 
                    {
                        ...
                        if (!dontCheck) 
                        {
                            ...
                            if (npt != null && npt.EndToken != newName.EndToken) 
                            {
                                ...
                                else if (MiscLocationHelper.CheckGeoObjectAfter(newName.EndToken, false)) 
                                {
                                }

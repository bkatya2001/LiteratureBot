...
namespace Pullenti.Ner.Geo
{
    ...
    public class GeoReferent : Pullenti.Ner.Referent
    {
        ...
        {
            if (((((((((((((v.Contains("поселок") || v.Contains("селение") || v.Contains("городок")) || v.Contains("село") || v.Contains("деревня")) || v.Contains("станица") || v.Contains("пункт")) || v.Contains("станция") || v.Contains("аул")) || v.Contains("хутор") || v.Contains("местечко")) || v.Contains("урочище") || v.Contains("усадьба")) || v.Contains("починок") || v.Contains("заимка")) || v.Contains("аал") || v.Contains("выселки")) || v.Contains("арбан") || v.Contains("місто")) || v.Contains("селище") || v.Contains("сіло")) || v.Contains("станиця") || v.Contains("станція")) || v.Contains("city") || v.Contains("municipality")) || v.Contains("town")) 
                return true;
        ...
        internal bool ContainsType(string typ)
        {
            foreach (Pullenti.Ner.Slot s in Slots) 
            {
                if (s.TypeName == ATTR_TYPE) 
                {
                    ...
                    if (ty == typ) 
                        return true;
                    if (ty.EndsWith(typ, StringComparison.OrdinalIgnoreCase)) 
                        return true;
                    if (typ.EndsWith(ty, StringComparison.OrdinalIgnoreCase)) 
                        return true;
                }
            }
            return false;
        ...
        internal void AddOrgReferent(Pullenti.Ner.Referent org)
        {
            ...
            foreach (Pullenti.Ner.Slot s in org.Slots) 
            {
                if (s.TypeName == "TYPE" && (s.Value is string)) 
                {
                    ...
                    if (pref == null) 
                        pref = ty;
                    else if (char.IsUpper(ty[0])) 
                    {
                        if (char.IsLower(pref[0])) 
                            pref = ty;
                        else if (pref.Length < ty.Length) 
                            pref = ty;
                    }
                    else if (char.IsLower(pref[0])) 
                    {
            ...
            foreach (Pullenti.Ner.Slot s in org.Slots) 
            {
                if (s.TypeName == "NAME") 
                {
                    string val = s.Value as string;
                    if (num != null) 
                        val = string.Format("{0}-{1}", val, num);
                    if (pref != null) 
                        this.AddName(string.Format("{0} {1}", pref.ToUpper(), val));
                    this.AddName(val);
                    nam = true;
                }

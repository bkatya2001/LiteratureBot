...
namespace Pullenti.Ner.Decree.Internal
{
    ...
    public class DecreeToken : Pullenti.Ner.MetaToken
    {
        ...
        public static DecreeToken TryAttach(Pullenti.Ner.Token t, DecreeToken prev = null, bool mustByTyp = false)
        {
            ...
            Pullenti.Ner.Core.AnalyzerData ad = Pullenti.Ner.Decree.DecreeAnalyzer.GetData(t);
            if (ad == null) 
                return null;
            if (ad.Level > 1) 
                return null;
            ...
            if (res == null) 
            {
                ...
                if ((t is Pullenti.Ner.TextToken) && (t as Pullenti.Ner.TextToken).Term == "ЗУ" && (t.WhitespacesAfterCount < 3)) 
                {
                    DecreeToken next = TryAttachName(t.Next, null, false, false);
                    bool ok = false;
                    if (next != null) 
                        ok = true;
                    else 
                    {
                        next = TryAttach(t.Next, null, false);
                        if (next != null && next.Typ == ItemType.Number) 
                        {
                            DecreeToken next2 = TryAttach(next.EndToken.Next, null, false);
                    ...
                    if (ok) 
                    {
                        res = new DecreeToken(t, t) { Typ = ItemType.Typ, Value = "ЗАКОН" };
                        return res;
                    }
                }
            ...
            if (res.Value == "КОДЕКС" && res.FullValue == null) 
            {
                ...
                for (Pullenti.Ner.Token tt = t1.Next; tt != null; tt = tt.Next) 
                {
                    ...
                    if (tt == t1.Next && npt.Morph.Case.IsGenitive) 
                    {
                        ...
                        {
                            Pullenti.Ner.Core.NounPhraseToken npt2 = Pullenti.Ner.Core.NounPhraseHelper.TryParse(tt.Next.Next, Pullenti.Ner.Core.NounPhraseParseAttr.No, 0, null);
                            if (npt2 != null && npt2.Morph.Case.IsGenitive) 
                            {
                                if (TryAttach(tt.Next.Next, null, false) == null) 
                                    t1 = (tt = npt2.EndToken);
                            }
                        }
                    }
        ...
        static DecreeToken _TryAttach(Pullenti.Ner.Token t, DecreeToken prev, int lev, bool mustByTyp = false)
        {
            ...
            if (!mustByTyp) 
            {
                ...
                if (tdat != null) 
                {
                    ...
                    Pullenti.Ner.ReferentToken dr = t.Kit.ProcessReferent("DATE", tdat, null);
                    if (dr != null) 
            ...
            if (toks != null) 
            {
                foreach (Pullenti.Ner.Core.TerminToken tok in toks) 
                {
                    ...
                    if ((tok.EndChar - tok.BeginChar) < 3) 
                    {
                        ...
                        {
                            if ((t.Next is Pullenti.Ner.ReferentToken) && (t.Next.GetReferent() is Pullenti.Ner.Org.OrganizationReferent)) 
                            {
                                string str = t.Next.GetReferent().ToString();
                                if (str.Contains("Союз")) 
                                {
                                }
                                else 
                                    return null;
            ...
            {
                num = true;
                if ((t1 is Pullenti.Ner.TextToken) && !t1.Chars.IsAllUpper && t1.LengthChar > 3) 
                {
        ...
        static Pullenti.Ner.Token _tryAttachNumber(Pullenti.Ner.Token t, StringBuilder tmp, bool afterNum)
        {
            ...
            for (; t2 != null; t2 = t2.Next) 
            {
                ...
                if (t2.IsWhitespaceAfter) 
                {
                    ...
                    if (((t2.Next != null && digs && t2.Next.IsHiphen) && (t2.Next.Next is Pullenti.Ner.TextToken) && (t2.Next.Next.LengthChar < 3)) && t2.Next.Next.Chars.IsLetter && Pullenti.Ner.Core.BracketHelper.CanBeStartOfSequence(t2.Next.Next.Next, false, false)) 
                    {
                        res = (t2 = t2.Next.Next);
                        tmp.AppendFormat("-{0}", t2.GetSourceText());
                        continue;
                    }
                    break;
                }
        ...
        public static List<DecreeToken> TryAttachList(Pullenti.Ner.Token t, DecreeToken prev = null, int maxCount = 10, bool mustStartByTyp = false)
        {
            ...
            for (; tt != null; tt = tt.Next) 
            {
                ...
                if ((p0 != null && p0.Typ == ItemType.Typ && p0.IsNewlineBefore) && res[0].Typ == ItemType.Typ) 
                    break;
                if (p0 == null) 
                {
                    if (tt.IsNewlineBefore) 
        ...
        public static DecreeToken TryAttachName(Pullenti.Ner.Token t, string typ, bool veryProbable = false, bool inTitleDocRef = false)
        {
            ...
            else 
            {
                ...
                if (ty != Pullenti.Ner.Decree.DecreeKind.Konvention && ty != Pullenti.Ner.Decree.DecreeKind.Program && ty != Pullenti.Ner.Decree.DecreeKind.Ustav) 
                    return null;
            }
            if (t == null) 
                return null;
            ...
            for (tt = t; tt != null; tt = tt.Next) 
            {
                ...
                if (r != null) 
                {
                    if (((r is Pullenti.Ner.Decree.DecreeReferent) || (r is Pullenti.Ner.Date.DateReferent) || (r is Pullenti.Ner.Org.OrganizationReferent)) || (r is Pullenti.Ner.Geo.GeoReferent) || r.TypeName == "NAMEDENTITY") 
                    {
                        if (tt.IsNewlineBefore) 
                            break;
                        if (ty == Pullenti.Ner.Decree.DecreeKind.Ustav || ty == Pullenti.Ner.Decree.DecreeKind.Kodex) 
                        {
                            if (r is Pullenti.Ner.Geo.GeoReferent) 
                                break;
                        }
            ...
            if (string.IsNullOrEmpty(s) || (s.Length < 4)) 
                return null;
            if ((s.Length < 10) && ty != Pullenti.Ner.Decree.DecreeKind.Kodex && ty != Pullenti.Ner.Decree.DecreeKind.Ustav) 
                return null;
            return new DecreeToken(t0, t1) { Typ = ItemType.Name, Value = s };
        }
        ...
        public static void Initialize()
        {
            ...
            m_Termins.Add(t);
            t = new Pullenti.Ner.Core.Termin("КОДЕКС") { Tag = ItemType.Typ };
            ...
            m_Termins.Add(t);
            t = new Pullenti.Ner.Core.Termin("КОДЕКС ПРО АДМІНІСТРАТИВНІ ПРАВОПОРУШЕННЯ", Pullenti.Morph.MorphLang.UA) { Tag = ItemType.Typ, Acronym = "КПАП", Tag2 = Pullenti.Ner.Decree.DecreeKind.Kodex };
        ...
        public static Pullenti.Ner.Decree.DecreeKind GetKind(string typ)
        {
            ...
            if (Pullenti.Morph.LanguageHelper.EndsWithEx(typ, "ЗАКОН", null, null, null)) 
                return Pullenti.Ner.Decree.DecreeKind.Law;
            if ((typ == "ПРИКАЗ" || typ == "УКАЗ" || typ == "РАСПОРЯЖЕНИЕ") || typ == "ПОСТАНОВЛЕНИЕ") 
                return Pullenti.Ner.Decree.DecreeKind.Order;
            if ((typ == "НАКАЗ" || typ == "УКАЗ" || typ == "РОЗПОРЯДЖЕННЯ") || typ == "ПОСТАНОВА") 
                return Pullenti.Ner.Decree.DecreeKind.Order;
        ...
        public static bool IsLaw(string typ)
        {
            ...
            if (ki == Pullenti.Ner.Decree.DecreeKind.Kodex || ki == Pullenti.Ner.Decree.DecreeKind.Law) 
                return true;
            return false;
        }
        public static bool IsJustice(string typ)
        {
            if ((((typ == "ОПРЕДЕЛЕНИЕ" || typ == "РЕШЕНИЕ" || typ == "ПОСТАНОВЛЕНИЕ") || typ == "ОПРЕДЕЛЕНИЕ" || typ == "УХВАЛА") || typ == "ВИЗНАЧЕННЯ" || typ == "РІШЕННЯ") || typ == "ПОСТАНОВА") 
                return true;
            return false;
        }
    }

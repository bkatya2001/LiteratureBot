    ...
    public class PersonPropertyReferent : Pullenti.Ner.Referent
    {
        ...
        public override bool CanBeEquals(Pullenti.Ner.Referent obj, Pullenti.Ner.Core.ReferentsEqualType typ)
        {
            ...
            else if (!eqBosses && n1 != n2) 
            {
                if (eqStart) 
                {
                    if (Higher != null && Higher.CanBeEquals(pr.Higher, typ)) 
                        return true;
                }
                return false;
            }
        ...
        public override bool CanBeGeneralFor(Pullenti.Ner.Referent obj)
        {
            ...
            foreach (Pullenti.Ner.Slot s in Slots) 
            {
                if (s.TypeName == ATTR_REF) 
                {
                    if (pr.FindSlot(ATTR_REF, s.Value, true) == null) 
                        return false;
                    refs++;
                }
            }
            ...
            if (n1 == n2) 
            {
                ...
                {
                    if (Higher != null) 
                        return Higher.CanBeGeneralFor(pr.Higher);
                    return false;
                }
                if (Higher != null) 
                {
                    if (!pr.Higher.CanBeEquals(Higher, Pullenti.Ner.Core.ReferentsEqualType.WithinOneText)) 
                        return false;
                }
                return true;
            }

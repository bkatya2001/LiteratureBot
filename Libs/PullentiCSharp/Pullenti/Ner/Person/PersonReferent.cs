...
namespace Pullenti.Ner.Person
{
    ...
    public class PersonReferent : Pullenti.Ner.Referent
    {
        ...
        /// </summary>
        public const string ATTR_UNDEFNAME = "UNDEFNAME";
        /// <summary>
        ...
        /// </summary>
        public const string ATTR_NAMETYPE = "NAMETYPE";
        /// <summary>
        ...
        public override string ToString(bool shortVariant, Pullenti.Morph.MorphLang lang = null, int lev = 0)
        {
            ...
            else 
            {
                ...
                if (niks.Count == 1) 
                    return string.Format("{0} ({1})", res, _outNik(niks[0]));
        ...
        static string _outNik(string str)
        {
            if (str.Length < 4) 
                return str;
        ...
        string toFullString(bool lastNameFirst, Pullenti.Morph.MorphLang lang, bool outNiks)
        {
            string id = null;
            foreach (Pullenti.Ner.Slot a in Slots) 
            {
            ...
            else if ((((n = this.GetStringValue(ATTR_FIRSTNAME)))) != null) 
            {
                ...
                if (outNiks) 
                {
                    ...
                    if (nik != null) 
                        n = string.Format("{0} {1}", n, _outNik(nik));
                }
                return n;
            }
            return "?";
        ...
        public override bool CanBeEquals(Pullenti.Ner.Referent obj, Pullenti.Ner.Core.ReferentsEqualType typ)
        {
            ...
            {
                List<string> und1 = this._getAllIdents();
                List<string> und2 = p._getAllIdents();
                if (und1.Count <= und2.Count) 
                {
                    foreach (string u in und1) 
                    {
                        if (!und2.Contains(u)) 
                        {
                            if (u == und1[und1.Count - 1] && _DelSurnameEnd(u) == _DelSurnameEnd(und2[und2.Count - 1])) 
                            {
                            }
                            else 
                                return false;
                ...
                {
                    foreach (string u in und2) 
                    {
                        if (!und1.Contains(u)) 
                        {
                            if (u == und2[und2.Count - 1] && _DelSurnameEnd(u) == _DelSurnameEnd(und1[und1.Count - 1])) 
                            {
                            }
                            return false;
                        }
        ...
        bool _checkNikEquSur(string nik, string sur)
        {
            if (nik == "I") 
        ...
        List<string> _getAllIdents()
        {
            List<string> res = this.GetStringValues(ATTR_UNDEFNAME);
            string s = this.GetStringValue(ATTR_FIRSTNAME);
            if (s != null) 
                res.Insert(0, s);
        ...
        {
            bool def = IsMale || IsFemale;
            for (int i = 0; i < Slots.Count; i++) 
            {
                if (Slots[i].TypeName == ATTR_LASTNAME && def) 
                {
                ...
                else if (Slots[i].TypeName == ATTR_UNDEFNAME) 
                {
                    if (this.FindSlot(ATTR_FIRSTNAME, Slots[i].Value, true) != null || this.FindSlot(ATTR_LASTNAME, Slots[i].Value, true) != null) 
                    {
                        Slots.RemoveAt(i);
                        i--;
                    }
            ...
            if (nik != null) 
            {
                for (int i = Slots.Count - 1; i >= 0; i--) 
                {
                    ...
                    {
                        if (this._checkNikEquSur(nik, Slots[i].Value as string)) 
                            Slots.RemoveAt(i);
                    }
                }
            }
        ...
        public override Pullenti.Ner.Core.IntOntologyItem CreateOntologyItem()
        {
            ...
            foreach (Pullenti.Ner.Slot a in Slots) 
            {
                ...
                else if (a.TypeName == ATTR_LASTNAME || a.TypeName == ATTR_FIRSTNAME || a.TypeName == ATTR_UNDEFNAME) 
                {
                    if (a.TypeName == ATTR_FIRSTNAME && this.FindSlot(ATTR_IDENTITY, null, true) == null) 
                        continue;

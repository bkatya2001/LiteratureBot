...
namespace Pullenti.Ner.Person.Internal
{
    class PersonIdToken : Pullenti.Ner.MetaToken
    {
        ...
        public static Pullenti.Ner.ReferentToken TryAttach(Pullenti.Ner.Token t)
        {
            ...
            else if (li.Count > 1 && li[0].Typ == Typs.Org && li[1].Typ == Typs.Number) 
            {
                i = 0;
                num = li[1].Value;
            }
        ...
        static PersonIdToken TryParse(Pullenti.Ner.Token t, PersonIdToken prev)
        {
                ...
                if (ty == Typs.Number) 
                {
                    ...
                    for (; tt != null; tt = tt.Next) 
                    {
                        if (tt.IsNewlineBefore) 
                            break;
                        if (!(tt is Pullenti.Ner.NumberToken) && !tt.IsCharOf("\\/")) 
                            break;
            ...
            if (npt != null) 
            {
                if (npt.EndToken.IsValue("УДОСТОВЕРЕНИЕ", "ПОСВІДЧЕННЯ")) 
                    return new PersonIdToken(t, npt.EndToken) { Typ = Typs.Keyword, Value = npt.GetNormalCaseText(null, Pullenti.Morph.MorphNumber.Undefined, Pullenti.Morph.MorphGender.Undefined, false) };
            }
            if (prev == null) 
                return null;

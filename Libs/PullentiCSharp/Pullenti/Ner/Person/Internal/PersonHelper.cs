...
namespace Pullenti.Ner.Person.Internal
{
    ...
    {
        internal static Pullenti.Ner.ReferentToken CreateReferentToken(Pullenti.Ner.Person.PersonReferent p, Pullenti.Ner.Token begin, Pullenti.Ner.Token end, Pullenti.Ner.MorphCollection morph, List<PersonAttrToken> attrs, bool forAttribute, bool afterBePredicate)
        {
            if (p == null) 
                return null;
            Pullenti.Ner.Token begin1 = begin;
            ...
            if ((begin.Previous != null && begin.Previous.IsCommaAnd && (begin.Previous.Previous is Pullenti.Ner.ReferentToken)) && (begin.Previous.Previous.GetReferent() is Pullenti.Ner.Person.PersonReferent)) 
            {
                ...
                {
                    bool ok = false;
                    Pullenti.Ner.Token tt1 = (rt00.BeginToken as Pullenti.Ner.ReferentToken).EndToken.Next;
                    if (rt00.BeginToken.Morph.Number == Pullenti.Morph.MorphNumber.Plural) 
                        ok = true;
                    if (tt1 != null && ((tt1.IsChar(':') || ((tt1.IsHiphen && p.FindSlot(Pullenti.Ner.Person.PersonReferent.ATTR_ATTR, null, true) == null))))) 
                        ok = true;
                    if (ok) 
                        p.AddSlot(Pullenti.Ner.Person.PersonReferent.ATTR_ATTR, rt00.BeginToken.GetReferent(), false, 0);
                }
            }
            if ((begin == end && (end.Next is Pullenti.Ner.TextToken) && end.Next.LengthChar == 2) && end.Next.Chars.IsAllUpper && (end.WhitespacesAfterCount < 3)) 
            {
                string nam = p.GetStringValue(Pullenti.Ner.Person.PersonReferent.ATTR_FIRSTNAME);
                string sec = p.GetStringValue(Pullenti.Ner.Person.PersonReferent.ATTR_MIDDLENAME);
                if (nam != null && sec != null) 
                {
                    if ((end.Next as Pullenti.Ner.TextToken).Term == string.Format("{0}{1}", nam[0], sec[0])) 
                        end = end.Next;
                }
            }
            PersonAnalyzerData ad = Pullenti.Ner.Person.PersonAnalyzer.GetData(begin);
            if (ad.Level > 3) 
                return new Pullenti.Ner.ReferentToken(p, begin, end) { Morph = morph, MiscAttrs = (int)p.m_PersonIdentityTyp };
            ad.Level++;
            ...
            for (Pullenti.Ner.Token t = end.Next; t != null; t = t.Next) 
            {
                ...
                if (t.IsValue("МОДЕЛЬ", null)) 
                    break;
                int cou1 = p.Slots.Count;
                Pullenti.Ner.Token tt = CorrectTailAttributes(p, t);
                if (tt != null && ((tt != t || p.Slots.Count != cou1))) 
                {
                    end = (t = tt);
                    continue;
                }
                ...
                if (t.IsChar('(') && t == end.Next) 
                {
                    ...
                    {
                        List<PersonItemToken> pits = PersonItemToken.TryAttachList(t, PersonItemToken.ParseAttr.CanBeLatin, 10);
                ...
                {
                    if (attr.Morph.Case.IsUndefined || attr.Morph.Case.IsNominative) 
                    {
                    }
                    else if (attr.Morph.Case.IsInstrumental && (t.Previous is Pullenti.Ner.TextToken) && (t.Previous as Pullenti.Ner.TextToken).IsVerbBe) 
                    {
                    }
                    else if ((t.Previous != null && t.Previous.IsCommaAnd && attrs1 != null) && t.Previous.Previous == attrs1[attrs1.Count - 1].EndToken) 
                    {
                    }
                    else 
                        break;
                ...
                if (openBr) 
                {
                    if (Pullenti.Ner.Person.PersonAnalyzer.TryAttachPerson(t, false, 0, true) != null) 
                        break;
                }
                if (attrs1 == null) 
                {
            ...
            if (attrs1 != null && !hasPrefix) 
            {
                ...
                {
                    Pullenti.Ner.ReferentToken rt = Pullenti.Ner.Person.PersonAnalyzer.TryAttachPerson(attr.BeginToken, false, -1, false);
                ...
                if (ok) 
                {
                    ...
                    {
                        Pullenti.Ner.ReferentToken rt1 = Pullenti.Ner.Person.PersonAnalyzer.TryAttachPerson(attr.BeginToken, false, -1, false);
                        if (rt1 != null) 
                            attrs1 = null;
                    }
                }
            ...
            for (Pullenti.Ner.Token t = end.Next; t != null; t = t.Next) 
            {
                ...
                if (!hasPrefix || crlfCou >= 2) 
                    break;
                Pullenti.Ner.ReferentToken rt = Pullenti.Ner.Person.PersonAnalyzer.ProcessReferentStat(t, null);
                if (rt != null) 
                    break;
            }
            ad.Level--;
            ...
            else 
            {
                ...
                for (; t != null; t = t.Next) 
                {
                    ...
                    if (t.WhitespacesBeforeCount > 2) 
                        break;
                    List<PersonItemToken> pli = PersonItemToken.TryAttachList(t, PersonItemToken.ParseAttr.No, 10);
        ...
        static Pullenti.Ner.Token CorrectTailAttributes(Pullenti.Ner.Person.PersonReferent p, Pullenti.Ner.Token t0)
        {
                ...
                if (r.TypeName == "DATE") 
                {
                    ...
                    else if (t.Next == null || t.IsNewlineAfter || t.Next.IsComma) 
                    {
                        ...
                        else 
                        {
                            ...
                            {
                                string str = r.ToString(true, null, 0);
                                int i = str.IndexOf(' ');
            ...
            {
                Pullenti.Ner.Core.NumberExToken nt = Pullenti.Ner.Core.NumberHelper.TryParseNumberWithPostfix(t);
                if (nt != null && nt.ExTyp == Pullenti.Ner.Core.NumberExType.Year) 
                {
                    if (nt.EndToken.IsNewlineAfter || nt.EndToken.Next.IsCommaAnd) 
                    {
                        if (p != null) 
                            p.AddSlot(Pullenti.Ner.Person.PersonReferent.ATTR_AGE, nt.Value, false, 0);
                        t = (res = nt.EndToken);
                    }
                }
            }

...
namespace Pullenti.Ner.Person.Internal
{
    public class PersonAttrToken : Pullenti.Ner.ReferentToken
    {
        public static void Initialize()
        {
            ...
            foreach (string s in new string[] {"ГОСПОДИН", "ГРАЖДАНИН", "УРОЖЕНЕЦ", "ВЫХОДЕЦ ИЗ", "МИСТЕР", "СЭР", "СЕНЬОР", "МОНСЕНЬОР", "СИНЬОР", "МЕСЬЕ", "МСЬЕ", "ДОН", "МАЭСТРО", "МЭТР"}) 
            {
            ...
            foreach (string s in new string[] {"ПАН", "ГРОМАДЯНИН", "УРОДЖЕНЕЦЬ", "ВИХОДЕЦЬ З", "МІСТЕР", "СЕР", "СЕНЬЙОР", "МОНСЕНЬЙОР", "МЕСЬЄ", "МЕТР", "МАЕСТРО"}) 
            {
            ...
            m_TerminsGrade = new Pullenti.Ner.Core.TerminCollection();
            ...
            {
                t = new PersonAttrTermin(grFull[i].ToUpper());
                t.AddAbridge(grSh1[i].ToUpper());
                t.AddAbridge(grSh2[i].ToUpper());
                m_TerminsGrade.Add(t);
            }
        }
        static Pullenti.Ner.Core.TerminCollection m_Termins;
        static PersonAttrTermin m_TerminVrio;
        static Pullenti.Ner.Core.TerminCollection m_TerminsGrade;
        ...
        internal static void PrepareAllData(Pullenti.Ner.Token t0)
        {
            if (!SpeedRegime) 
                return;
            PersonAnalyzerData ad = Pullenti.Ner.Person.PersonAnalyzer.GetData(t0);
            if (ad == null) 
                return;
            for (Pullenti.Ner.Token t = t0; t != null; t = t.Next) 
            {
                PersonTokenData d = t.Tag as PersonTokenData;
                PersonAttrToken typ = TryAttach(t, PersonAttrAttachAttrs.No);
                if (typ != null) 
                {
                    if (d == null) 
        ...
        public static PersonAttrToken TryAttach(Pullenti.Ner.Token t, PersonAttrAttachAttrs attrs = PersonAttrAttachAttrs.No)
        {
            if (t == null) 
                return null;
            PersonAnalyzerData ad = Pullenti.Ner.Person.PersonAnalyzer.GetData(t);
            if (ad == null) 
                return null;
            ...
            {
                PersonAttrToken res1 = _TryAttach(res.EndToken.Next, attrs);
                if (res1 != null) 
                {
            ...
            if (res.Typ == PersonAttrTerminType.Prefix && ((((((res.Value == "ГРАЖДАНИН" || res.Value == "ГРАЖДАНКА" || res.Value == "УРОЖЕНЕЦ") || res.Value == "УРОЖЕНКА" || res.Value == "ВЫХОДЕЦ ИЗ") || res.Value == "ВИХОДЕЦЬ З" || res.Value == "ГРОМАДЯНИН") || res.Value == "ГРОМАДЯНКА" || res.Value == "УРОДЖЕНЕЦЬ") || res.Value == "УРОДЖЕНКА")) && res.EndToken.Next != null) 
            {
                ...
                if (r != null && r.TypeName == ObjNameGeo) 
                {
                    ...
                    {
                        if (Pullenti.Ner.Geo.Internal.GeoOwnerHelper.CanBeHigher(r as Pullenti.Ner.Geo.GeoReferent, res.EndToken.Next.GetReferent() as Pullenti.Ner.Geo.GeoReferent, null, null)) 
                        {
                ...
                {
                    PersonAttrToken aaa = _TryAttach(tt.Next, attrs);
            ...
            if (res.Typ == PersonAttrTerminType.King || res.Typ == PersonAttrTerminType.Position) 
            {
                if (res.BeginToken == res.EndToken && res.Chars.IsCapitalUpper && res.WhitespacesAfterCount == 1) 
                {
                    ...
                    {
                        Pullenti.Ner.ReferentToken rt1 = Pullenti.Ner.Person.PersonAnalyzer.ProcessReferentStat(t.Next, null);
            ...
            {
                PersonItemToken pat = PersonItemToken.TryAttach(res.BeginToken, PersonItemToken.ParseAttr.IgnoreAttrs, null);
            ...
            {
                List<PersonItemToken> pits = PersonItemToken.TryAttachList(res.EndToken.Next, PersonItemToken.ParseAttr.No, 10);
            ...
            {
                Pullenti.Ner.ReferentToken rt1 = res.Kit.ProcessReferent("ORGANIZATION", res.EndToken.Next, null);
            ...
            {
                Pullenti.Ner.ReferentToken r = res.Kit.ProcessReferent("GEO", res.BeginToken, null);
                if (r != null) 
                {
            ...
            if (res.PropRef != null && res.PropRef.Kind == Pullenti.Ner.Person.PersonPropertyKind.King) 
            {
                ...
                if (t1 != null && t1.Chars.IsCapitalUpper && (t1.WhitespacesBeforeCount < 3)) 
                {
                    ...
                    {
                        if (((tt.IsValue("ВСЕЙ", null) || tt.IsValue("ВСЕЯ", "ВСІЄЇ"))) && (tt.Next is Pullenti.Ner.ReferentToken) && (tt.Next.GetReferent() is Pullenti.Ner.Geo.GeoReferent)) 
                        {
                            adjs.Add(Pullenti.Ner.Core.MiscHelper.ConvertFirstCharUpperAndOtherLower(Pullenti.Ner.Core.MiscHelper.GetTextValue(tt, tt.Next, Pullenti.Ner.Core.GetTextAttr.No)));
                            t2 = (tt = tt.Next);
                        }
                        else if ((tt.GetReferent() is Pullenti.Ner.Geo.GeoReferent) && (tt as Pullenti.Ner.ReferentToken).EndToken.Morph.Class.IsAdjective) 
                        {
                        ...
                        else 
                        {
                            ...
                            if (pit.Lastname == null) 
                                ...
                                foreach (PersonItemToken.MorphPersonItemVariant fv in pit.Lastname.Vars) 
                                {
                    ...
                    else if (t2 != null) 
                    {
                        ...
                        {
                            if (i > 0) 
                                res.PropRef.Name = string.Format("{0}{1}", res.PropRef.Name, ((i + 1) < adjs.Count ? "," : " и"));
                            res.PropRef.Name = string.Format("{0} {1}", res.PropRef.Name, adjs[i]);
                        }
                        res.EndToken = t2;
                    }
                }
            ...
            if (res.EndToken.WhitespacesAfterCount < 2) 
            {
                ...
                else if (te != null && te.IsChar('(')) 
                {
                    ...
                    {
                        Pullenti.Ner.ReferentToken rt1 = te.Kit.ProcessReferent("DATE", te, null);
                ...
                Pullenti.Ner.Core.NounPhraseToken npt = Pullenti.Ner.Core.NounPhraseHelper.TryParse(res.BeginToken, Pullenti.Ner.Core.NounPhraseParseAttr.No, 0, null);
                    {
                        ...
                        else if (res.WhitespacesAfterCount == 1) 
                        {
                            ...
                            {
                                Pullenti.Ner.Token t1 = res.EndToken.Next;
                                Pullenti.Ner.ReferentToken pr = Pullenti.Ner.Person.PersonAnalyzer.TryAttachPerson(t1, false, 0, true);
                                if (pr != null && res.CanHasPersonAfter == 1) 
                                {
                                    if (pr.BeginToken == t1) 
                                    {
                                        ...
                                        {
                                            if (Pullenti.Ner.Person.PersonAnalyzer.TryAttachPerson(pr.EndToken.Next, false, 0, true) != null) 
                                            {
                                            }
                                            else 
                                                pr = null;
                                ...
                                {
                                    List<PersonItemToken> pits = PersonItemToken.TryAttachList(t1, PersonItemToken.ParseAttr.No, 10);
                                    ...
                                    else if (pits != null && pits.Count == 3 && ((pits[2].Middlename != null || ((pits[1].Typ == PersonItemToken.ItemType.Initial && pits[2].Typ == PersonItemToken.ItemType.Initial))))) 
                                    {
                                        Pullenti.Ner.Token tt = pits[2].EndToken.Next;
                                        if (tt != null && ((tt.IsHiphen || tt.IsChar('(')))) 
                                        {
                                            List<PersonItemToken> pits2 = PersonItemToken.TryAttachList(tt.Next, PersonItemToken.ParseAttr.No, 10);
                                            if (pits2 != null && pits2.Count > 0) 
                                            {
                                            }
                                            else 
                                                pr = null;
            ...
            {
                PersonAttrToken rnext = TryAttach(res.EndToken.Next, PersonAttrAttachAttrs.No);
        ...
        static PersonAttrToken _TryAttach(Pullenti.Ner.Token t, PersonAttrAttachAttrs attrs)
        {
            if (t == null) 
                return null;
            if (t.Morph.Class.IsPronoun && (((t.IsValue("ЕГО", "ЙОГО") || t.IsValue("ЕЕ", "ЇЇ") || t.IsValue("HIS", null)) || t.IsValue("HER", null)))) 
            {
                PersonAttrToken res1 = TryAttach(t.Next, attrs);
            ...
            if (tt == null) 
            {
                ...
                if (nt != null) 
                {
                    ...
                    {
                        PersonAttrToken pat0 = _TryAttach(t.Next, attrs);
                ...
                {
                    PersonAttrToken res0 = _TryAttach(t.Next.Next.Next, attrs);
            ...
            {
                PersonAttrToken res0 = _TryAttach(t.Next, attrs);
                if (res0 != null) 
                {
                    res0.BeginToken = t;
                    return res0;
            ...
            for (int step = 0; step < 2; step++) 
            {
                ...
                else if (toks == null && (t is Pullenti.Ner.TextToken) && t.Morph.Language.IsEn) 
                {
                    ...
                    {
                        Pullenti.Ner.ReferentToken rt = Pullenti.Ner.Person.PersonAnalyzer.ProcessReferentStat(t.Next, null);
                ...
                if ((toks == null && step == 0 && t.Chars.IsLatinLetter) && (t.WhitespacesAfterCount < 2)) 
                {
                    ...
                    {
                        List<PersonItemToken> pits = PersonItemToken.TryAttachList(t, PersonItemToken.ParseAttr.CanBeLatin | PersonItemToken.ParseAttr.IgnoreAttrs, 10);
                    ...
                    foreach (Pullenti.Ner.Core.TerminToken tok in toks) 
                    {
                        ...
                        if (pat.Typ == PersonAttrTerminType.Position || pat.Typ == PersonAttrTerminType.Prefix || pat.Typ == PersonAttrTerminType.King) 
                        {
                            ...
                            if (res != null) 
                            {
                                ...
                                {
                                    bool ok = false;
                                    pit = PersonItemToken.TryAttach(pit.EndToken.Next, PersonItemToken.ParseAttr.IgnoreAttrs, null);
                                    if (pit != null && pit.Typ == PersonItemToken.ItemType.Initial) 
                                    {
                                        pit = PersonItemToken.TryAttach(pit.EndToken.Next, PersonItemToken.ParseAttr.IgnoreAttrs, null);
                                        if (pit != null && pit.Typ == PersonItemToken.ItemType.Initial) 
                                    ...
                                    if (!ok) 
                                    {
                                        if (_TryAttach(tok.EndToken.Next, attrs) != null) 
                                            ok = true;
                                    }
                                    if (!ok) 
                                        return null;
        ...
        static PersonAttrToken CreateAttrPosition(Pullenti.Ner.Core.TerminToken tok, PersonAttrAttachAttrs attrs)
        {
                    ...
                    else 
                    {
                        ...
                        if (pat == null) 
                        {
                            ...
                            if (npt == null || npt.EndToken == tok.EndToken.Next) 
                                continue;
                            pat = TryAttach(npt.EndToken, PersonAttrAttachAttrs.No);
                    ...
                    for (; ts != null; ts = ts.Next) 
                    {
                        if (ts.Morph.Class.IsPreposition) 
                        {
                            if (ts.IsValue("В", null) || ts.IsValue("ПО", null)) 
                            {
                                ...
                                Pullenti.Ner.ReferentToken rt11 = ts.Kit.ProcessReferent("NAMEDENTITY", ts.Next, null);
                                if (rt11 != null) 
                                {
            ...
            {
                PersonAttrToken pat = _TryAttach(tok.EndToken.Next, attrs);
            ...
            {
                if (t1.Next.Next.Chars.Equals(t1.Chars) || m_Termins.TryParse(t1.Next.Next, Pullenti.Ner.Core.TerminParseAttr.No) != null || ((t1.Next.Next.Chars.IsAllLower && t1.Next.Next.Chars.IsCyrillicLetter))) 
                {
            ...
            for (Pullenti.Ner.Token t = t1.Next; t != null; t = t.Next) 
            {
                ...
                if (pat != null) 
                {
                    ...
                    else if ((!tok.Morph.Case.IsGenitive && (tok.Termin is PersonAttrTermin) && (tok.Termin as PersonAttrTermin).CanHasPersonAfter == 1) && pat.Morph.Case.IsGenitive) 
                    {
                        ...
                        if (!t.Kit.MiscData.ContainsKey("IgnorePersons")) 
                        {
                            ...
                            rr = Pullenti.Ner.Person.PersonAnalyzer.ProcessReferentStat(t, null);
                            if (t.Kit.MiscData.ContainsKey("IgnorePersons")) 
                    ...
                    {
                        Pullenti.Ner.ReferentToken ppp = Pullenti.Ner.Person.PersonAnalyzer.ProcessReferentStat(pat.EndToken.Next.Next, null);
                        if (ppp != null) 
                        {
                            t1 = (tname1 = (t = pat.EndToken));
                            continue;
                ...
                if (r != null) 
                {
                    ...
                    if (r.TypeName == ObjNameGeo && (tok.Termin as PersonAttrTermin).Typ == PersonAttrTerminType.King && (te as Pullenti.Ner.ReferentToken).EndToken.Morph.Class.IsAdjective) 
                        break;
                    if ((r.TypeName == ObjNameGeo || r.TypeName == ObjNameAddr || r.TypeName == "DATERANGE") || r.TypeName == ObjNameOrg || r.TypeName == ObjNameTransport) 
                    {
                        ...
                        bool posol = ((r.TypeName == ObjNameGeo || r.TypeName == ObjNameOrg)) && Pullenti.Morph.LanguageHelper.EndsWithEx(name, "посол", "представитель", "консул", "представник");
                        if (posol) 
                        {
                            t = t1;
                            continue;
                        ...
                        if (r.TypeName == ObjNameOrg) 
                        {
                            ...
                            {
                                if (((t is Pullenti.Ner.TextToken) && t.LengthChar >= 2 && t.Chars.IsAllUpper) && (t.WhitespacesBeforeCount < 2)) 
                                {
                                    t1 = t;
                                    continue;
                                }
                            ...
                            for (; t != null; t = t.Next) 
                            {
                                ...
                                if (((t is Pullenti.Ner.TextToken) && t.LengthChar > 2 && t.Previous.IsValue("В", null)) && tname1 == null && t.Next != null) 
                                {
                                    Pullenti.Ner.ReferentToken rt = Pullenti.Ner.Person.PersonAnalyzer.ProcessReferentStat(t.Next, null);
                                    if (rt != null || (t.Next.GetReferent() is Pullenti.Ner.Person.PersonReferent)) 
                                    {
                                        tname0 = t.Previous;
                                        tname1 = (t1 = t);
                                        continue;
                ...
                {
                    PersonItemToken pit = PersonItemToken.TryAttach(t, PersonItemToken.ParseAttr.CanBeLatin | PersonItemToken.ParseAttr.IgnoreAttrs, null);
                    if (pit != null) 
                    {
                        ...
                        if (pit.Firstname != null && pit.Firstname.IsInDictionary) 
                            break;
                        if ((tok.Termin as PersonAttrTermin).Typ == PersonAttrTerminType.King) 
                        {
                        ...
                        List<PersonItemToken> pits = PersonItemToken.TryAttachList(t, PersonItemToken.ParseAttr.No | PersonItemToken.ParseAttr.IgnoreAttrs, 6);
                        if (pits != null && pits.Count > 0) 
                        {
                            if (pits.Count == 2) 
                            {
                            ...
                            if ((tok.Termin as PersonAttrTermin).Typ == PersonAttrTerminType.King) 
                            {
                                if (pits[0].Morph.Case.IsUndefined && !pits[0].Morph.Case.IsGenitive) 
                                    break;
                                if (pits.Count == 1) 
                                {
                                    Pullenti.Morph.MorphClass mc1 = pits[0].BeginToken.GetMorphClassInDictionary();
                                    if (mc1.IsUndefined) 
                                        break;
                                }
                            }
                ...
                if (!t.Chars.IsAllLower) 
                {
                    ...
                    {
                        List<PersonItemToken> pits = PersonItemToken.TryAttachList(t, PersonItemToken.ParseAttr.IgnoreAttrs, 10);
                ...
                if (test) 
                {
                    Pullenti.Ner.ReferentToken rto = t.Kit.ProcessReferent("ORGANIZATION", t, null);
                    if (rto != null) 
                    {
                    ...
                    if (rto != null && rto.EndChar >= t.EndChar && rto.BeginChar == t.BeginChar) 
                    {
                        ...
                        {
                            Pullenti.Ner.ReferentToken rto2 = t.Kit.ProcessReferent("ORGANIZATION", t.Next.Next, null);
                    ...
                    if (rto != null && rto.Morph.Case.IsGenitive) 
                    {
                        pr.AddSlot(Pullenti.Ner.Person.PersonPropertyReferent.ATTR_REF, rto.Referent, false, 0);
                        pr.AddExtReferent(rto);
                        t = (t1 = rto.EndToken);
                ...
                if (t.Morph.Class.IsPreposition) 
                {
                    ...
                    {
                        if (Pullenti.Ner.Core.BracketHelper.CanBeStartOfSequence(npt.EndToken.Next, true, false)) 
                        {
                            ...
                            {
                                Pullenti.Ner.Core.BracketSequenceToken br = Pullenti.Ner.Core.BracketHelper.TryParse(npt.EndToken.Next, Pullenti.Ner.Core.BracketParseAttr.No, 100);
                                if (br != null) 
                                {
                                    t = (t1 = br.EndToken);
                                    continue;
                ...
                if (t.Morph.Class.IsPronoun) 
                    continue;
                if ((t.IsHiphen && tname1 == t.Previous && !t.IsWhitespaceBefore) && !t.IsWhitespaceAfter && (t.Next is Pullenti.Ner.TextToken)) 
                {
                    t = t.Next;
                    t1 = (tname1 = t);
                    continue;
        ...
        static bool _isPerson(Pullenti.Ner.Token t)
        {
            ...
            if (!t.Chars.IsLetter || t.Chars.IsAllLower) 
                return false;
            Pullenti.Ner.ReferentToken rt00 = Pullenti.Ner.Person.PersonAnalyzer.ProcessReferentStat(t, null);
        ...
        static PersonAttrToken CreateAttrGrade(Pullenti.Ner.Core.TerminToken tok)
        {
            ...
            if (t1 == null) 
                return null;
            Pullenti.Ner.Person.PersonPropertyReferent pr = new Pullenti.Ner.Person.PersonPropertyReferent();
            string ss = t1.Tag as string;
            if (ss == null) 
                ss = "наук";
            pr.Name = string.Format("{0} {1}", tok.Termin.CanonicText.ToLower(), ss);
        ...
        {
            int i = 0;
            Pullenti.Ner.Core.TerminToken te = m_TerminsGrade.TryParse(t, Pullenti.Ner.Core.TerminParseAttr.No);
            if (te != null) 
            {
                te.EndToken.Tag = te.Termin.CanonicText.ToLower();
                return te.EndToken;
            }
            ...
            for (; t != null; t = t.Next) 
            {
                if (t.IsValue("НАУК", null)) 
                {
                    if (tt.Next == t && tt.GetMorphClassInDictionary().IsAdjective) 
                        t.Tag = Pullenti.Ner.Core.MiscHelper.GetTextValue(tt, t, Pullenti.Ner.Core.GetTextAttr.No).ToLower();
                    t1 = t;
                    i++;
                    break;
                }

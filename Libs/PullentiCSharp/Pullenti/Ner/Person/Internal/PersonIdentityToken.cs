...
namespace Pullenti.Ner.Person.Internal
{
    class PersonIdentityToken : Pullenti.Ner.MetaToken
    {
        ...
        public static Pullenti.Ner.Person.PersonReferent TryAttachOntoForDuble(PersonItemToken pit0, PersonItemToken pit1)
        {
            if ((pit0 == null || pit0.Firstname == null || pit1 == null) || pit1.Middlename == null) 
                return null;
            PersonAnalyzerData ad = Pullenti.Ner.Person.PersonAnalyzer.GetData(pit0);
            if (ad.LocalOntology.Items.Count > 100) 
                return null;
            Pullenti.Ner.Person.PersonReferent p0 = null;
            int cou = 0;
            foreach (Pullenti.Ner.Core.IntOntologyItem p in ad.LocalOntology.Items) 
            {
                if (p.Referent != null) 
                {
            ...
            if (otl != null) 
            {
                foreach (Pullenti.Ner.Core.IntOntologyToken ot in otl) 
                {
                    if (ot.EndToken == pits[ind].EndToken) 
                    {
                        ...
                        if (!ot.Morph.Case.IsUndefined && inf0.Case.Equals(Pullenti.Morph.MorphCase.AllCases) && ot.BeginToken == ot.EndToken) 
                        ...
                        if ((pit == null && pits.Count == 1 && vars.Count == 0) && pers.FindSlot(null, pits[0].Value, true) != null) 
                        {
                            Pullenti.Ner.Person.PersonReferent p = new Pullenti.Ner.Person.PersonReferent();
                            p.AddSlot(Pullenti.Ner.Person.PersonReferent.ATTR_UNDEFNAME, pits[0].Value, false, 0);
                            pit = new PersonIdentityToken(pits[0].BeginToken, pits[0].EndToken) { Coef = 3, Typ = FioTemplateType.Undefined };
                            pit.OntologyPerson = p;
                        ...
                        else if (pit == null) 
                        {
                            ...
                            if (pit == null || (pit.Coef < 2)) 
                                continue;
                            if (pit.EndToken != pits[pits.Count - 1].EndToken && vars.Count > 0) 
                                continue;
                            Pullenti.Ner.Person.PersonReferent p = new Pullenti.Ner.Person.PersonReferent();
        ...
        public static List<PersonIdentityToken> TryAttach(List<PersonItemToken> pits, int ind, Pullenti.Morph.MorphBaseInfo inf, Pullenti.Ner.Token firstTok, bool king, bool isAttrBefore)
        {
            ...
            foreach (PersonIdentityToken r in res) 
            {
                ...
                if (ttt != null && ttt.Morph.Class.Equals(Pullenti.Morph.MorphClass.Verb)) 
                {
        ...
        public static void ManageLastname(PersonIdentityToken res, PersonItemToken pit, Pullenti.Morph.MorphBaseInfo inf)
        {
            ...
            else 
            {
                ...
                if (res.Lastname.Items.Count == 0 && pit.Value != null) 
                    res.Lastname.Items.Add(new PersonMorphCollection.PersonMorphVariant() { Value = pit.Value });
            ...
            if (pit.Firstname == null) 
            {
                ...
                if (pit.Value.IndexOf('-') > 0 && pit.EndToken.GetMorphClassInDictionary().IsProperName) 
                {
                    if (pit.Lastname != null) 
                        ...
                        res.Firstname.Items.Add(new PersonMorphCollection.PersonMorphVariant() { Value = pit.Value });
                }
                else 
                {
                    if (pit.Lastname != null) 
        ...
        static PersonIdentityToken TryAttachNameSurname(List<PersonItemToken> pits, int ind, Pullenti.Morph.MorphBaseInfo inf, bool prevHasThisTyp = false, bool isAttrBefore = false)
        {
            ...
            {
                if (pits[ind + 1].BeginToken is Pullenti.Ner.ReferentToken) 
                    res.Coef++;
                ...
                {
                    Pullenti.Morph.MorphClass mc1 = pits[ind].BeginToken.GetMorphClassInDictionary();
                    if (!mc1.IsNoun) 
                        res.Coef++;
                }
            }
            ...
            else if (ind == 0 && pits.Count == 2 && pits[1].IsInDictionary) 
            {
            ...
            if (ind == 0 && pits.Count == 2 && pits[1].Middlename != null) 
            {
                Pullenti.Ner.Token t0 = pits[0].BeginToken;
                if (t0.Previous != null && t0.Previous.IsComma) 
                    t0 = t0.Previous;
                if ((t0.Previous is Pullenti.Ner.TextToken) && t0.Previous.IsNewlineBefore) 
                {
                    Pullenti.Ner.Mail.Internal.MailLine li = Pullenti.Ner.Mail.Internal.MailLine.Parse(t0.Previous, 0, 0);
                    if (li != null && li.Typ == Pullenti.Ner.Mail.Internal.MailLine.Types.Hello) 
                        return null;
                }
            }
            return res;
        ...
        static PersonIdentityToken TryAttachNameSecnameSurname(List<PersonItemToken> pits, int ind, Pullenti.Morph.MorphBaseInfo inf, bool prevHasThisTyp = false)
        {
            ...
            else if (pits[ind + 1].Typ != PersonItemToken.ItemType.Initial && !pits[ind].Chars.Equals(pits[ind + 1].Chars)) 
                res.Coef -= 1;
        ...
        static void CorrectCoefAfterLastname(PersonIdentityToken res, List<PersonItemToken> pits, int ind)
        {
            if (!pits[ind - 1].IsNewlineAfter) 
            {
                PersonAttrToken pat = PersonAttrToken.TryAttach(pits[ind - 1].BeginToken, PersonAttrToken.PersonAttrAttachAttrs.OnlyKeyword);
                if (pat != null) 
                    res.Coef -= 1;
            }
            if (ind >= pits.Count) 
        ...
        static PersonIdentityToken TryAttachSurnameNameSecname(List<PersonItemToken> pits, int ind, Pullenti.Morph.MorphBaseInfo inf, bool prevHasThisTyp = false, bool always = false)
        {
            ...
            if (pits[ind].IsInDictionary && pits[ind + 1].IsInDictionary && pits[ind + 2].IsInDictionary) 
                res.Coef--;
            else if (pits[ind].IsInDictionary && pits[ind].Chars.IsCapitalUpper && Pullenti.Ner.Core.MiscHelper.CanBeStartOfSentence(pits[ind].BeginToken)) 
            {
                Pullenti.Morph.MorphClass mc = pits[ind].BeginToken.GetMorphClassInDictionary();
        ...
        static PersonIdentityToken TryAttachSurnameII(List<PersonItemToken> pits, int ind, Pullenti.Morph.MorphBaseInfo inf)
        {
            ...
            else if (pits[ind].WhitespacesAfterCount == pits[i1].WhitespacesBeforeCount && pits[i1].Lastname != null) 
            {
                ...
                else 
                {
                    if (pits[i1].IsInDictionary && pits[i1].BeginToken.GetMorphClassInDictionary().IsNoun) 
                    {
                        ...
                        res.Coef--;
                    bool ok = true;
                    for (Pullenti.Ner.Token tt = pits[ind].BeginToken; tt != null; tt = tt.Previous) 
                    {
                        ...
                        else if ((tt is Pullenti.Ner.TextToken) && tt.Chars.IsLetter && tt != pits[ind].BeginToken) 
                        {
                            ok = false;
                            break;
                        }
            ...
            {
                if (((pits[0].BeginToken.Morph.Number & Pullenti.Morph.MorphNumber.Plural)) != Pullenti.Morph.MorphNumber.Plural) 
                    res.Coef--;
        ...
        static PersonIdentityToken TryAttachIISurname(List<PersonItemToken> pits, int ind, Pullenti.Morph.MorphBaseInfo inf)
        {
            ...
            if (!pits[ind].IsNewlineBefore) 
            {
                ...
                {
                    prev = PersonItemToken.TryAttach(pits[ind].BeginToken.Previous, (pits[i1].Chars.IsLatinLetter ? PersonItemToken.ParseAttr.CanBeLatin : PersonItemToken.ParseAttr.No), null);
                    if (prev != null) 
                    {
        ...
        static PersonIdentityToken TryAttachKing(List<PersonItemToken> pits, int ind, Pullenti.Morph.MorphBaseInfo inf, bool prevHasThisTyp = false)
        {
            ...
            if (num < 1) 
            {
                if (((pits[0].Firstname != null || pits.Count == 1)) && prevHasThisTyp) 
                {
                    if (pits.Count == 1) 
                        ok = true;
                    else if (pits.Count == 2 && ((pits[0].EndToken.Next.IsHiphen || pits[1].Firstname != null))) 
                        ok = true;
                }
                if (!ok) 
                    return null;
            ...
            if (num > 0) 
            {
                ...
                {
                    List<PersonItemToken> pits1 = PersonItemToken.TryAttachList(numt.Next, PersonItemToken.ParseAttr.No, 2);
                    if (pits1 != null && pits1.Count == 1 && pits1[0].Firstname != null) 
                    {
                        ManageMiddlename(res, pits1[0], inf);
        ...
        static PersonIdentityToken TryAttachArabic(List<PersonItemToken> pits, int ind, Pullenti.Morph.MorphBaseInfo inf, bool prevHasThisTyp = false)
        {
            if (ind > 0 || (pits.Count < 4)) 
                return null;
            int i;
            bool hasHiph = false;
            int errs = 0;
            for (i = 0; i < pits.Count; i++) 
            {
                PersonItemToken p = pits[i];
            ...
            if ((i < 4) || errs > 1) 
                return null;
            if ((((i < 5) || i != pits.Count)) && !hasHiph) 
            {
                if (((pits.Count == 4 && i == pits.Count && pits[0].Firstname != null) && pits[1].Firstname != null && pits[2].Firstname != null) && pits[3].Firstname == null && pits[3].Middlename == null) 
                {
                }
                else 
                    return null;
            }
            PersonIdentityToken res = new PersonIdentityToken(pits[0].BeginToken, pits[i - 1].EndToken) { Typ = FioTemplateType.ArabicLong };
            ...
            if (pits[0].Firstname != null) 
            {
                ...
                res.Lastname.Items.Add(new PersonMorphCollection.PersonMorphVariant() { Value = pits[0].Value });
            for (int j = 1; j < i; j++) 
            {
                PersonItemToken p = pits[j];
                foreach (PersonMorphCollection.PersonMorphVariant v in res.Lastname.Items) 
                {
                    v.Value = string.Format("{0} {1}", v.Value, p.Value);
                }
            }
            return res;
        }
        static PersonIdentityToken TryAttachAsian(List<PersonItemToken> pits, int ind, Pullenti.Morph.MorphBaseInfo inf, int cou, bool prevHasThisTyp = false)
        {
            ...
            Pullenti.Morph.MorphClass mc0 = pits[0].GetMorphClassInDictionary();
        ...
        static Pullenti.Ner.MorphCollection AccordMorph(Pullenti.Morph.MorphBaseInfo inf, PersonItemToken.MorphPersonItem p1, PersonItemToken.MorphPersonItem p2, PersonItemToken.MorphPersonItem p3, Pullenti.Ner.Token next)
        {
            ...
            if ((next != null && (next is Pullenti.Ner.TextToken) && next.Chars.IsAllLower) && next.Morph.Class.Equals(Pullenti.Morph.MorphClass.Verb) && next.Morph.Number == Pullenti.Morph.MorphNumber.Singular) 
            {
        ...
        public static bool CorrectXFML(List<PersonIdentityToken> pli0, List<PersonIdentityToken> pli1, List<PersonAttrToken> attrs)
        {
            ...
            {
                if (PersonItemToken.TryAttach(p1.EndToken.Next, PersonItemToken.ParseAttr.No, null) != null) 
                    return false;
            }

...
namespace Pullenti.Ner.Person
{
    ...
    public class PersonAnalyzer : Pullenti.Ner.Analyzer
    {
        ...
        {
            return new Pullenti.Ner.Person.Internal.PersonAnalyzerData();
        }
        internal static Pullenti.Ner.Person.Internal.PersonAnalyzerData GetData(Pullenti.Ner.Token t)
        {
            if (t == null) 
                return null;
            return t.Kit.GetAnalyzerDataByAnalyzerName(ANALYZER_NAME) as Pullenti.Ner.Person.Internal.PersonAnalyzerData;
        }
        public override void Process(Pullenti.Ner.Core.AnalysisKit kit)
        {
            Pullenti.Ner.Person.Internal.PersonAnalyzerData ad = kit.GetAnalyzerData(this) as Pullenti.Ner.Person.Internal.PersonAnalyzerData;
            ...
            Pullenti.Ner.Person.Internal.PersonAttrToken.SpeedRegime = false;
            Pullenti.Ner.Person.Internal.PersonAttrToken.PrepareAllData(kit.FirstToken);
            ad.ARegime = true;
            ...
            for (int step = 0; step < steps; step++) 
            {
                ...
                for (Pullenti.Ner.Token t = kit.FirstToken; t != null; t = t.Next) 
                {
                    ...
                    List<Pullenti.Ner.ReferentToken> rts = this.TryAttachPersons(t, step);
                    if (rts != null) 
                    {
            ...
            List<PersonPropertyReferent> genAttrs = new List<PersonPropertyReferent>();
            for (Pullenti.Ner.Token t = kit.FirstToken; t != null; t = t.Next) 
            {
                if (t.IsIgnored || !(t is Pullenti.Ner.ReferentToken)) 
                    continue;
                PersonReferent p = t.GetReferent() as PersonReferent;
                if (p == null) 
                    continue;
                ...
                {
                    PersonPropertyReferent pr = tt.GetReferent() as PersonPropertyReferent;
                    if (pr == null) 
                        continue;
                    Pullenti.Ner.Slot s0 = p.FindSlot(PersonReferent.ATTR_ATTR, pr, true);
                    foreach (Pullenti.Ner.Slot s in p.Slots) 
                    {
                        if (s.TypeName == PersonReferent.ATTR_ATTR && s != s0) 
                        {
                            PersonPropertyReferent pr1 = s.Value as PersonPropertyReferent;
                            if (pr1 == null) 
                                continue;
                            if (!pr.CanBeGeneralFor(pr1)) 
                                continue;
                            (tt as Pullenti.Ner.ReferentToken).Referent = pr1;
                            if (pr1.GeneralReferent != null) 
                                pr1.GeneralReferent = null;
                            pr1.AddOccurenceOfRefTok(tt as Pullenti.Ner.ReferentToken);
                            if (!genAttrs.Contains(pr)) 
                                genAttrs.Add(pr);
                            foreach (Pullenti.Ner.TextAnnotation o in pr.Occurrence) 
                            {
                            ...
                            PersonPropertyReferent hi = pr.Higher;
                            if (hi != null) 
                            {
                                if (pr1.Higher != null && hi.CanBeGeneralFor(pr1.Higher)) 
                                {
                                    if (!genAttrs.Contains(hi)) 
                                        genAttrs.Add(hi);
                                    foreach (Pullenti.Ner.TextAnnotation o in hi.Occurrence) 
                                    {
            ...
            foreach (PersonPropertyReferent g in genAttrs) 
            {
                if (g.Occurrence.Count == 0) 
                    ad.RemoveReferent(g);
            }
            ...
            for (Pullenti.Ner.Token t = kit.FirstToken; t != null; t = t.Next) 
            {
                ...
                if (pers.Count == 1) 
                {
                    ...
                    {
                        Pullenti.Ner.ReferentToken rt1 = new Pullenti.Ner.ReferentToken(ppr, pat, pat) { Morph = pat.Morph };
                        kit.EmbedToken(rt1);
                        Pullenti.Ner.ReferentToken rt2 = new Pullenti.Ner.ReferentToken(pers[0], rt1, rt1) { Morph = pat.Morph };
                        kit.EmbedToken(rt2);
                        continue;
                    }
                }
        ...
        public static Pullenti.Ner.ReferentToken ProcessReferentStat(Pullenti.Ner.Token begin, string param = null)
        {
            if (begin == null) 
                return null;
            Pullenti.Ner.Person.Internal.PersonAnalyzerData ad = GetData(begin);
            if (ad == null || ad.Level > 1) 
                return null;
            begin.Kit.FixAnalyzer("ORGANIZATION", true);
            ad.Level++;
            Pullenti.Ner.ReferentToken rt = TryAttachPerson(begin, false, -1, false);
            ad.Level--;
            ...
            ad.Level++;
            begin.Kit.FixAnalyzer("ORGANIZATION", true);
            Pullenti.Ner.Person.Internal.PersonAttrToken pat = Pullenti.Ner.Person.Internal.PersonAttrToken.TryAttach(begin, Pullenti.Ner.Person.Internal.PersonAttrToken.PersonAttrAttachAttrs.No);
            begin.Kit.FixAnalyzer("ORGANIZATION", false);
            ad.Level--;
        ...
        List<Pullenti.Ner.ReferentToken> TryAttachPersons(Pullenti.Ner.Token t, int step)
        {
            Pullenti.Ner.ReferentToken rt = TryAttachPerson(t, false, step, false);
            if (rt == null) 
                return null;
            ...
            for (Pullenti.Ner.Token tt = rt.EndToken.Next; tt != null; tt = tt.Next) 
            {
                if (!tt.IsCommaAnd) 
                    break;
                List<Pullenti.Ner.Person.Internal.PersonItemToken> pits = Pullenti.Ner.Person.Internal.PersonItemToken.TryAttachList(tt.Next, Pullenti.Ner.Person.Internal.PersonItemToken.ParseAttr.No, 10);
                if (pits == null || pits.Count != 1) 
                    break;
                Pullenti.Ner.ReferentToken rt1 = TryAttachPerson(tt.Next, false, step, false);
                if (rt1 != null) 
                    break;
        ...
        internal static Pullenti.Ner.ReferentToken TryAttachPerson(Pullenti.Ner.Token t, bool forExtOntos, int step, bool forAttribute = false)
        {
            ...
            Pullenti.Ner.Person.Internal.PersonAnalyzerData ad = PersonAnalyzer.GetData(t);
            if (ad == null) 
                return null;
            mi.Case = ((forExtOntos || ad.NominativeCaseAlways) ? Pullenti.Morph.MorphCase.Nominative : Pullenti.Morph.MorphCase.AllCases);
            ...
            for (; t != null; t = (t == null ? null : t.Next)) 
            {
                ...
                if (t.Chars.IsLatinLetter && step == 0) 
                {
                    ...
                    if (Pullenti.Ner.Core.MiscHelper.IsEngArticle(t)) 
                        tt2 = t.Next;
                    Pullenti.Ner.Person.Internal.PersonItemToken pit0 = Pullenti.Ner.Person.Internal.PersonItemToken.TryAttach(tt2, Pullenti.Ner.Person.Internal.PersonItemToken.ParseAttr.CanBeLatin, null);
                    if (pit0 != null && Pullenti.Ner.Core.MiscHelper.IsEngAdjSuffix(pit0.EndToken.Next) && ad != null) 
                    {
                        ...
                        if (pp != null) 
                            return Pullenti.Ner.Person.Internal.PersonHelper.CreateReferentToken(pp, pit0.BeginToken, pit0.EndToken, pit0.Morph, attrs, forAttribute, afterBePredicate);
                    }
                }
                Pullenti.Ner.Person.Internal.PersonAttrToken a = null;
                if ((step < 1) || t.InnerBool) 
                {
                    a = Pullenti.Ner.Person.Internal.PersonAttrToken.TryAttach(t, Pullenti.Ner.Person.Internal.PersonAttrToken.PersonAttrAttachAttrs.No);
                    if (step == 0 && a != null) 
                        t.InnerBool = true;
                }
                if ((a != null && a.BeginToken == a.EndToken && !a.BeginToken.Chars.IsAllLower) && (a.WhitespacesAfterCount < 3)) 
                {
                    List<Pullenti.Ner.Person.Internal.PersonItemToken> pits = Pullenti.Ner.Person.Internal.PersonItemToken.TryAttachList(t, Pullenti.Ner.Person.Internal.PersonItemToken.ParseAttr.IgnoreAttrs, 10);
                ...
                {
                    Pullenti.Ner.Person.Internal.PersonItemToken pit = Pullenti.Ner.Person.Internal.PersonItemToken.TryAttach(t, Pullenti.Ner.Person.Internal.PersonItemToken.ParseAttr.CanBeLatin, null);
                ...
                {
                    if (!a.Morph.Case.IsUndefined && !mi.Case.IsUndefined) 
                    {
                        if (((a.Morph.Case & mi.Case)).IsUndefined) 
                            return null;
                    }
                    if ((t.Previous.IsAnd && attrs.Count == 1 && attrs[0].PropRef != null) && a.PropRef != null) 
                    {
                        Pullenti.Ner.Referent r1 = attrs[0].PropRef.GetSlotValue(PersonPropertyReferent.ATTR_REF) as Pullenti.Ner.Referent;
                        Pullenti.Ner.Referent r2 = a.PropRef.GetSlotValue(PersonPropertyReferent.ATTR_REF) as Pullenti.Ner.Referent;
                        if ((r1 is Pullenti.Ner.Geo.GeoReferent) && (r2 is Pullenti.Ner.Geo.GeoReferent)) 
                        {
                            if (!r1.CanBeEquals(r2, Pullenti.Ner.Core.ReferentsEqualType.WithinOneText)) 
                                return null;
                        }
                    }
                }
                ...
                if (a.CanHasPersonAfter >= 0) 
                {
                    ...
                    if (!a.Morph.Case.IsUndefined && a.CanHasPersonAfter == 0) 
            ...
            {
                Pullenti.Ner.ReferentToken pr = TryAttachPerson(t.Next, forExtOntos, step, forAttribute);
                if (pr != null && pr.EndToken.Next != null && pr.EndToken.Next.IsChar(')')) 
                {
                    Pullenti.Ner.ReferentToken res = Pullenti.Ner.Person.Internal.PersonHelper.CreateReferentToken(pr.Referent as PersonReferent, t, pr.EndToken.Next, attrs[0].Morph, attrs, true, afterBePredicate);
                    if (res != null) 
                        res.EndToken = pr.EndToken.Next;
                    return res;
                }
                Pullenti.Ner.Person.Internal.PersonAttrToken attr = Pullenti.Ner.Person.Internal.PersonAttrToken.TryAttach(t.Next, Pullenti.Ner.Person.Internal.PersonAttrToken.PersonAttrAttachAttrs.No);
            ...
            if (mi.Case.Equals(Pullenti.Morph.MorphCase.AllCases) && tt0 != null) 
            {
            ...
            for (int k = 0; k < 2; k++) 
            {
                    ...
                    if (pits[0].EndToken.Next != null && pits[0].EndToken.Next.IsAnd && (pits[0].EndToken.Next.Next is Pullenti.Ner.ReferentToken)) 
                    {
                        ...
                        if (pr != null) 
                        {
                            ...
                            if (pers.Slots.Count == 0) 
                                return null;
                            pers.AddSlot(PersonReferent.ATTR_FIRSTNAME, v.Value, false, 0);
                            return Pullenti.Ner.Person.Internal.PersonHelper.CreateReferentToken(pers, pits[0].BeginToken, pits[0].EndToken, pits[0].Firstname.Morph, attrs, forAttribute, afterBePredicate);
                        }
                    }
                ...
                if (pits != null && pits.Count == 1 && pits[0].Lastname != null) 
                {
                    if (t.Morph.Number == Pullenti.Morph.MorphNumber.Plural || ((t.Previous != null && ((t.Previous.IsValue("БРАТ", null) || t.Previous.IsValue("СЕСТРА", null)))))) 
                    {
                        ...
                        if (t1 != null && ((t1.IsChar(':') || t1.IsHiphen))) 
                            t1 = t1.Next;
                        List<Pullenti.Ner.Person.Internal.PersonItemToken> pits1 = Pullenti.Ner.Person.Internal.PersonItemToken.TryAttachList(t1, pattr, 10);
                ...
                if (forAttribute && pits.Count > 1) 
                {
                    ...
                    if (pit0 != null) 
                        return Pullenti.Ner.Person.Internal.PersonHelper.CreateReferentToken(pit0.OntologyPerson, pit0.BeginToken, pit0.EndToken, pit0.Morph, attrs, forAttribute, afterBePredicate);
                }
                for (int i = 0; (i < pits.Count) && (i < 3); i++) 
                {
                    ...
                    if (pit != null) 
                        return Pullenti.Ner.Person.Internal.PersonHelper.CreateReferentToken(pit.OntologyPerson, pit.BeginToken, pit.EndToken, pit.Morph, (pit.BeginToken == pits[0].BeginToken ? attrs : null), forAttribute, afterBePredicate);
                }
                if (pits.Count == 1 && !forExtOntos && attrs == null) 
                {
                    PersonReferent pp = Pullenti.Ner.Person.Internal.PersonIdentityToken.TryAttachOntoForSingle(pits[0], ad.LocalOntology);
                    if (pp != null) 
                        return Pullenti.Ner.Person.Internal.PersonHelper.CreateReferentToken(pp, pits[0].BeginToken, pits[0].EndToken, pits[0].Morph, attrs, forAttribute, afterBePredicate);
                }
                ...
                {
                    PersonReferent pp = Pullenti.Ner.Person.Internal.PersonIdentityToken.TryAttachOntoForDuble(pits[0], pits[1]);
                    if (pp != null) 
                        return Pullenti.Ner.Person.Internal.PersonHelper.CreateReferentToken(pp, pits[0].BeginToken, pits[1].EndToken, pits[0].Morph, attrs, forAttribute, afterBePredicate);
                }
                if (pits[0].BeginToken.Kit.Ontology != null) 
                {
                    for (int i = 0; i < pits.Count; i++) 
                    {
                        ...
                        if (pit != null) 
                            return Pullenti.Ner.Person.Internal.PersonHelper.CreateReferentToken(pit.OntologyPerson, pit.BeginToken, pit.EndToken, pit.Morph, attrs, forAttribute, afterBePredicate);
                    }
                }
                ...
                if (pli0.Count > 0 && pli0[0].Typ == Pullenti.Ner.Person.Internal.FioTemplateType.NameSurname) 
                {
                    ...
                    {
                        List<Pullenti.Ner.Person.Internal.PersonItemToken> pits1 = Pullenti.Ner.Person.Internal.PersonItemToken.TryAttachList(attrs[attrs.Count - 1].BeginToken, pattr, 10);
                ...
                if ((((pli == null || pli.Count == 0)) && pits.Count == 1 && pits[0].Lastname != null) && attrs != null && !pits[0].IsInDictionary) 
                {
                    foreach (Pullenti.Ner.Person.Internal.PersonAttrToken a in attrs) 
                    {
                        ...
                        {
                            Pullenti.Morph.MorphClass mc = pits[0].BeginToken.GetMorphClassInDictionary();
                            if (!mc.IsProper) 
                                break;
                ...
                if (pli != null && pli.Count > 0) 
                {
                    ...
                    if ((best.Coef < minCoef) && ((attrs != null || forExtOntos))) 
                    {
                        ...
                        if (pit != null && pit.Coef > best.Coef && pit.Coef > 0) 
                        {
                            ...
                            return Pullenti.Ner.Person.Internal.PersonHelper.CreateReferentToken(pers, pit.BeginToken, pit.EndToken, pit.Morph, attrs, forAttribute, afterBePredicate);
                        }
                    ...
                    if (best.Coef >= 0 && (best.Coef < minCoef)) 
                    {
                        ...
                        Pullenti.Ner.Person.Internal.PersonAttrToken att = Pullenti.Ner.Person.Internal.PersonAttrToken.TryAttach(tee, Pullenti.Ner.Person.Internal.PersonAttrToken.PersonAttrAttachAttrs.No);
                        if (att == null && tee1 != null) 
                            att = Pullenti.Ner.Person.Internal.PersonAttrToken.TryAttach(tee1, Pullenti.Ner.Person.Internal.PersonAttrToken.PersonAttrAttachAttrs.No);
                        if (att != null) 
                        {
                    ...
                    if ((pits.Count >= 3 && best.Typ == Pullenti.Ner.Person.Internal.FioTemplateType.IISurname && (best.Coef < minCoef)) && best.Coef > 1 && pits[2].Value != null) 
                    {
                        List<Pullenti.Ner.Core.IntOntologyToken> aaa = ad.LocalOntology.TryAttach(pits[2].BeginToken, null, false);
                        if (aaa != null && aaa.Count > 0) 
                            best.Coef += 1;
                    }
                    if (((best.Coef >= 0 && (best.Coef < minCoef) && attrs == null) && best.BeginToken.Previous != null && best.BeginToken.Previous.IsCommaAnd) && (best.BeginToken.Previous.Previous is Pullenti.Ner.ReferentToken)) 
                    {
                        PersonReferent ppp = best.BeginToken.Previous.Previous.GetReferent() as PersonReferent;
                    ...
                    if (best.Coef >= minCoef) 
                    {
                                ...
                                if (pli[0].Lastname.Values.Count == 2) 
                                {
                                    ...
                                    for (Pullenti.Ner.Token ttt = pli[0].EndToken.Next; ttt != null && cou > 0; ttt = ttt.Next,cou--) 
                                    {
                                        ...
                                        if (!ttt.IsValue(sur, null)) 
                                            continue;
                                        Pullenti.Ner.ReferentToken pr = TryAttachPerson(ttt, forExtOntos, step, false);
                        ...
                        foreach (Pullenti.Ner.Person.Internal.PersonIdentityToken v in pli) 
                        {
                            ...
                            else if (v.Typ == Pullenti.Ner.Person.Internal.FioTemplateType.AsianName || v.Typ == Pullenti.Ner.Person.Internal.FioTemplateType.ArabicLong) 
                        ...
                        return Pullenti.Ner.Person.Internal.PersonHelper.CreateReferentToken(pers, pli[0].BeginToken, (latin != null ? latin.EndToken : pli[0].EndToken), pli[0].Morph, attrs, forAttribute, afterBePredicate);
                    }
                }
            }
            if (attrs != null) 
        ...
        public override Pullenti.Ner.ReferentToken ProcessOntologyItem(Pullenti.Ner.Token begin)
        {
            if (begin == null) 
                return null;
            Pullenti.Ner.ReferentToken rt = TryAttachPerson(begin, true, -1, false);
            if (rt == null) 
            {
                Pullenti.Ner.Person.Internal.PersonAttrToken pat = Pullenti.Ner.Person.Internal.PersonAttrToken.TryAttach(begin, Pullenti.Ner.Person.Internal.PersonAttrToken.PersonAttrAttachAttrs.No);
                if (pat != null && pat.PropRef != null) 
            ...
            for (; t != null; t = t.Next) 
            {
                ...
                {
                    Pullenti.Ner.ReferentToken rt1 = TryAttachPerson(t.Next, true, -1, false);

    ...
    public class MeasureToken : Pullenti.Ner.MetaToken
    {
        ...
        public static MeasureToken TryParseMinimal(Pullenti.Ner.Token t, Pullenti.Ner.Core.TerminCollection addUnits, bool canOmitNumber = false)
        {
            if (t == null || (t is Pullenti.Ner.ReferentToken)) 
                return null;
            List<NumbersWithUnitToken> mt = NumbersWithUnitToken.TryParseMulti(t, addUnits, (canOmitNumber ? NumberWithUnitParseAttr.CanOmitNumber : NumberWithUnitParseAttr.No));
            if (mt == null) 
                return null;
        ...
        void _parseInternals(Pullenti.Ner.Core.TerminCollection addUnits)
        {
            if (EndToken.Next != null && ((EndToken.Next.IsCharOf("\\/") || EndToken.Next.IsValue("ПРИ", null)))) 
            {
                ...
                {
                    NumbersWithUnitToken mt = NumbersWithUnitToken.TryParse(EndToken.Next.Next, addUnits, NumberWithUnitParseAttr.No);
        ...
        public static MeasureToken TryParse(Pullenti.Ner.Token t, Pullenti.Ner.Core.TerminCollection addUnits, bool canBeSet = true, bool canUnitsAbsent = false, bool isResctriction = false, bool isSubval = false)
        {
            ...
            {
                Pullenti.Ner.Date.Internal.DateItemToken dtok = Pullenti.Ner.Date.Internal.DateItemToken.TryParse(t, null, false);
                if (dtok != null) 
                    return null;
            }
            ...
            for (tt = t1.Next; tt != null; tt = tt.Next) 
            {
                ...
                if (tt.IsValue("ПРИ", null)) 
                {
                    ...
                    NumbersWithUnitToken n1 = NumbersWithUnitToken.TryParse(tt.Next, addUnits, NumberWithUnitParseAttr.No);
            ...
            if (not) 
                attrs |= NumberWithUnitParseAttr.Not;
            if (isResctriction) 
                attrs |= NumberWithUnitParseAttr.IsSecond;
            List<NumbersWithUnitToken> mts = NumbersWithUnitToken.TryParseMulti(t1, addUnits, attrs);
            if (mts == null) 
            {
            ...
            if (t1 != null && t1.IsCommaAnd) 
                t1 = t1.Next;
            List<NumbersWithUnitToken> mts1 = NumbersWithUnitToken.TryParseMulti(t1, addUnits, NumberWithUnitParseAttr.No);

...
namespace Pullenti.Ner.Measure.Internal
{
    ...
    public class NumbersWithUnitToken : Pullenti.Ner.MetaToken
    {
        ...
        public static List<NumbersWithUnitToken> TryParseMulti(Pullenti.Ner.Token t, Pullenti.Ner.Core.TerminCollection addUnits, NumberWithUnitParseAttr attrs = NumberWithUnitParseAttr.No)
        {
            ...
            if (tt0.IsChar('(')) 
            {
                ...
                if (whd != null) 
                    tt0 = whd.EndToken;
                List<NumbersWithUnitToken> res0 = TryParseMulti(tt0.Next, addUnits, attrs);
                if (res0 != null) 
                {
                    res0[0].WHL = whd;
            ...
            if ((!mt.IsWhitespaceAfter && MeasureHelper.IsMultCharEnd(mt.EndToken.Next) && (mt.EndToken.Next.Next is Pullenti.Ner.NumberToken)) && mt.Units.Count == 0) 
            {
                ...
                if (terms != null && terms.Count > 0) 
                {
                    ...
                    List<NumbersWithUnitToken> res1 = TryParseMulti(mt.EndToken.Next, addUnits, NumberWithUnitParseAttr.No);
                    if (res1 != null) 
                    {
                        res1.Insert(0, mt);
                        return res1;
        ...
        public static NumbersWithUnitToken TryParse(Pullenti.Ner.Token t, Pullenti.Ner.Core.TerminCollection addUnits, NumberWithUnitParseAttr attrs = NumberWithUnitParseAttr.No)
        {
            if (t == null) 
                return null;
            NumbersWithUnitToken res = _tryParse(t, addUnits, attrs);
            if (res != null && ((attrs & NumberWithUnitParseAttr.Not)) == NumberWithUnitParseAttr.Not) 
                res.Not = true;
            return res;
        }
        ...
        internal static NumbersWithUnitToken _tryParse(Pullenti.Ner.Token t, Pullenti.Ner.Core.TerminCollection addUnits, NumberWithUnitParseAttr attrs)
        {
            if (t == null) 
                return null;
            while (t != null) 
            ...
            if (t0.IsChar('(')) 
            {
                NumbersWithUnitToken mt0 = _tryParse(t.Next, addUnits, NumberWithUnitParseAttr.No);
            ...
            if (tok != null) 
            {
                if (t.IsValue("С", null)) 
                {
                    Pullenti.Ner.Core.NounPhraseToken npt = Pullenti.Ner.Core.NounPhraseHelper.TryParse(t.Next, Pullenti.Ner.Core.NounPhraseParseAttr.ParseNumericAsAdjective, 0, null);
                ...
                if (t != null && t.IsChar('(')) 
                {
                    ...
                    if (uni != null) 
                    {
                        ...
                        NumbersWithUnitToken mt0 = _tryParse(t, addUnits, attrs & NumberWithUnitParseAttr.CanOmitNumber);
            ...
            bool second = ((attrs & NumberWithUnitParseAttr.IsSecond)) != NumberWithUnitParseAttr.No;
            if (t != null) 
            {
            ...
            if (num == null) 
            {
                ...
                if (uni != null) 
                {
                    ...
                    if (!delim) 
                    {
                        if (t == null) 
                        {
                            if (hasKeyw && ((attrs & ((NumberWithUnitParseAttr.CanBeNon | NumberWithUnitParseAttr.CanOmitNumber)))) != NumberWithUnitParseAttr.No) 
                            {
                            }
                            else 
                                return null;
            ...
            if (num == null) 
            {
                ...
                {
                    if (((attrs & NumberWithUnitParseAttr.CanOmitNumber)) == NumberWithUnitParseAttr.No && !hasKeyw && ((attrs & NumberWithUnitParseAttr.CanBeNon)) == NumberWithUnitParseAttr.No) 
                        return null;
                    ...
                    else if (hasKeyw && ((attrs & NumberWithUnitParseAttr.CanBeNon)) != NumberWithUnitParseAttr.No) 
                    {
                        rval = double.NaN;
            ...
            else 
            {
                ...
                rval = num.RealValue;
                if (res.EndToken.Next != null && res.EndToken.Next.IsChar('(') && (res.EndToken.Next.Next is Pullenti.Ner.NumberToken)) 
                {
                    Pullenti.Ner.NumberToken nn = res.EndToken.Next.Next as Pullenti.Ner.NumberToken;
            ...
            if (uni == null) 
            {
                ...
                if (uni != null) 
                {
                    ...
                    t = res.EndToken.Next;
                    if ((t != null && t.IsValue("И", null) && t.Next != null) && dty == DiapTyp.Undefined) 
                    {
                        if (_tryParse(t.Next, addUnits, attrs) == null) 
                        {
                            if (t.Next.IsValue("БОЛЕЕ", null) || t.Next.IsValue("БОЛЬШЕ", null)) 
                            {
                                dty = DiapTyp.Ge;
            ...
            else 
            {
                ...
                if (uni.Count > 1) 
                {
                    ...
                    {
                        NumbersWithUnitToken num2 = _tryParse(uni1[uni1.Count - 1].EndToken.Next.Next, addUnits, NumberWithUnitParseAttr.No);
            ...
            NumbersWithUnitToken next = null;
            ...
            else 
            {
                ...
                if (((attrs & NumberWithUnitParseAttr.CanBeNon)) != NumberWithUnitParseAttr.No) 
                    a |= NumberWithUnitParseAttr.CanBeNon;
                next = _tryParse(t, addUnits, a);
            }
            ...
            if (next != null && ((next.ToVal != null || next.SingleVal != null)) && next.FromVal == null) 
            {
                ...
                {
                    NumbersWithUnitToken next2 = _tryParse(next.EndToken.Next.Next.Next, addUnits, NumberWithUnitParseAttr.IsSecond);
        ...
        internal static void Initialize()
        {
            ...
            Pullenti.Ner.Core.Termin t = new Pullenti.Ner.Core.Termin("МЕНЕЕ") { Tag = DiapTyp.Ls };
            foreach (string s in lss) 
            {
                t.AddVariant(s, false);
            }
            m_Termins.Add(t);
            t = new Pullenti.Ner.Core.Termin("НЕ МЕНЕЕ") { Tag = DiapTyp.Ge };
            foreach (string s in lss) 
            {

    ...
    class OrgItemNameToken : Pullenti.Ner.MetaToken
    {
        ...
        public static OrgItemNameToken TryAttach(Pullenti.Ner.Token t, OrgItemNameToken prev, bool extOnto, bool first)
        {
            ...
            if (t.IsValue("ОРДЕНА", null) && t.Next != null) 
            {
                ...
                Pullenti.Ner.ReferentToken ppp = t.Kit.ProcessReferent("PERSON", t.Next, null);
                if (ppp != null) 
            ...
            if (prev == null && !extOnto) 
            {
                ...
                {
                    OrgAnalyzerData ad = t.Kit.Ontology._getAnalyzerData(Pullenti.Ner.Org.OrganizationAnalyzer.ANALYZER_NAME) as OrgAnalyzerData;
                    if (ad != null) 
                    {
            ...
            if (m_StdNouns.TryParse(res.EndToken, Pullenti.Ner.Core.TerminParseAttr.No) != null) 
            {
                ...
                for (Pullenti.Ner.Token tt = res.EndToken.Next; tt != null; tt = tt.Next) 
                {
                    ...
                    if (npt2 == null) 
                    {
                        if (dd.Equals(Pullenti.Morph.MorphClass.Adjective)) 
                            continue;
                        break;
                    }
                    ...
                    {
                        Pullenti.Ner.ReferentToken rt = Pullenti.Ner.Org.OrganizationAnalyzer.ProcessReferentStat(tt, null);
                        if (rt != null) 
                            break;
                    }
                    cou++;
        ...
        static OrgItemNameToken _TryAttach(Pullenti.Ner.Token t, OrgItemNameToken prev, bool extOnto)
        {
            ...
            if (tt.IsChar(',') && prev != null) 
            {
                ...
                if (npt1 == null || !npt1.Chars.Equals(prev.Chars) || ((npt1.Morph.Case & prev.Morph.Case)).IsUndefined) 
                    return null;
                OrgItemTypeToken ty = OrgItemTypeToken.TryAttach(t.Next, false);
                if (ty != null) 
                    return null;
                ...
                if (npt2 == null || !npt2.Chars.Equals(prev.Chars) || ((npt2.Morph.Case & npt1.Morph.Case & prev.Morph.Case)).IsUndefined) 
                    return null;
            ...
            if (npt != null && ((explOk || npt.Morph.Case.IsGenitive || ((prev != null && !((prev.Morph.Case & npt.Morph.Case)).IsUndefined))))) 
            {
                ...
                if ((npt.EndToken.WhitespacesAfterCount < 2) && (npt.EndToken.Next is Pullenti.Ner.TextToken)) 
                {
                    ...
                    {
                        OrgItemTypeToken typ = OrgItemTypeToken.TryAttach(npt.EndToken.Next, true);
                        OrgItemEponymToken epo = OrgItemEponymToken.TryAttach(npt.EndToken.Next, false);
                        Pullenti.Ner.ReferentToken rtt = t.Kit.ProcessReferent("PERSONPROPERTY", npt.EndToken.Next, null);
                ...
                if (prev.Morph.Number != Pullenti.Morph.MorphNumber.Undefined && res.Morph.Number != Pullenti.Morph.MorphNumber.Undefined) 
                {
                    if (((prev.Morph.Number & res.Morph.Number)) == Pullenti.Morph.MorphNumber.Undefined) 
                    {
                        if (!prev.Chars.Equals(res.Chars)) 
                            return null;
                        OrgItemTypeToken ty = OrgItemTypeToken.TryAttach(res.EndToken.Next, false);
                        if (ty != null) 
                            return null;
                    }
                }
                ...
                if (npt != null) 
                {
                    ...
                    else if (tt.Term == "ПРИ") 
                    {
                        ...
                        if (ok) 
                        {
                            ...
                            {
                                Pullenti.Ner.ReferentToken rt = Pullenti.Ner.Org.OrganizationAnalyzer.ProcessReferentStat(tt.Next, null);
                                if (rt != null) 
                                    ok = false;
                            }
                        }

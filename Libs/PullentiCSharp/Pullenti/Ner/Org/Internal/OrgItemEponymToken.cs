...
namespace Pullenti.Ner.Org.Internal
{
    public class OrgItemEponymToken : Pullenti.Ner.MetaToken
    {
        ...
        public static OrgItemEponymToken TryAttach(Pullenti.Ner.Token t, bool mustHasPrefix = false)
        {
            ...
            {
                Pullenti.Ner.ReferentToken rt = t1.Kit.ProcessReferent("PERSON", t1, null);
            ...
            {
                if (its.Count == (i + 2) && its[i].Chars.Equals(its[i + 1].Chars)) 
                {
            ...
            else if (its[i].Typ == PersonItemType.Name && holy) 
            {
                t1 = its[i].EndToken;
                bool sec = false;
                if (((i + 1) < its.Count) && its[i].Chars.Equals(its[i + 1].Chars) && its[i + 1].Typ != PersonItemType.Initial) 
                {
                    sec = true;
                    t1 = its[i + 1].EndToken;
                }

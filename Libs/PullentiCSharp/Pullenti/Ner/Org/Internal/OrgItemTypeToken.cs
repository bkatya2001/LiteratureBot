...
namespace Pullenti.Ner.Org.Internal
{
    public class OrgItemTypeToken : Pullenti.Ner.MetaToken
    {
        ...
        {
            OrgItemTypeToken res = new OrgItemTypeToken(BeginToken, EndToken);
            res.Morph = Morph;
            res.Typ = Typ;
            res.Name = Name;
            res.AltName = AltName;
        ...
        public static OrgItemTypeToken TryAttachPureKeywords(Pullenti.Ner.Token t)
        {
        ...
        internal static void PrepareAllData(Pullenti.Ner.Token t0)
        {
            if (!SpeedRegime) 
                return;
            OrgAnalyzerData ad = Pullenti.Ner.Org.OrganizationAnalyzer.GetData(t0);
            if (ad == null) 
                return;
            for (Pullenti.Ner.Token t = t0; t != null; t = t.Next) 
            {
                OrgTokenData d = t.Tag as OrgTokenData;
                OrgItemTypeToken typ = TryAttach(t, false);
                if (typ != null) 
                {
                    if (d == null) 
        ...
        internal static void RecalcData(Pullenti.Ner.Token t)
        {
            if (!SpeedRegime) 
                return;
            OrgAnalyzerData ad = Pullenti.Ner.Org.OrganizationAnalyzer.GetData(t);
            if (ad == null) 
                return;
            bool r = ad.TRegime;
            ad.TRegime = false;
        ...
        public static OrgItemTypeToken TryAttach(Pullenti.Ner.Token t, bool canBeFirstLetterLower = false)
        {
            if (t == null) 
                return null;
            OrgAnalyzerData ad = Pullenti.Ner.Org.OrganizationAnalyzer.GetData(t);
        ...
        static OrgItemTypeToken TryAttachInt(Pullenti.Ner.Token t, bool canBeFirstLetterLower)
        {
            if (t is Pullenti.Ner.ReferentToken) 
            {
                ...
                else if ((t.GetReferent() is Pullenti.Ner.Geo.GeoReferent) && (t as Pullenti.Ner.ReferentToken).EndToken.Morph.Class.IsAdjective) 
                {
                }
                else 
                    return null;
            ...
            {
                res = _TryAttach(t.Next, canBeFirstLetterLower, false);
                if (res == null) 
                    return null;
            ...
            if (((res == null && t.Chars.IsCapitalUpper && (t.Next is Pullenti.Ner.TextToken)) && (t.WhitespacesAfterCount < 3) && (t.Next as Pullenti.Ner.TextToken).Term == "РБ") && t.Morph.Class.IsAdjective && ((t.Morph.Gender & Pullenti.Morph.MorphGender.Feminie)) != Pullenti.Morph.MorphGender.Undefined) 
            {
                res = new OrgItemTypeToken(t, t.Next);
                res.Typ = "больница";
                res.Name = string.Format("{0} РАЙОННАЯ БОЛЬНИЦА", t.GetNormalCaseText(Pullenti.Morph.MorphClass.Adjective, Pullenti.Morph.MorphNumber.Singular, Pullenti.Morph.MorphGender.Feminie, false));
                res.Coef = 3;
                res.Profiles.Add(Pullenti.Ner.Org.OrgProfile.Medicine);
            }
            if (res == null && t.Chars.IsLatinLetter) 
            {
                if (t.IsValue("THE", null)) 
                {
                    OrgItemTypeToken res1 = TryAttach(t.Next, canBeFirstLetterLower);
                    if (res1 != null) 
                    {
                        res1.BeginToken = t;
                        return res1;
                ...
                if (t.Chars.IsCapitalUpper && (t is Pullenti.Ner.TextToken)) 
                {
            ...
            if ((res != null && res.Name != null && res.Name.StartsWith("СОВМЕСТ")) && Pullenti.Morph.LanguageHelper.EndsWithEx(res.Name, "ПРЕДПРИЯТИЕ", "КОМПАНИЯ", null, null)) 
            {
                ...
                {
                    Pullenti.Ner.ReferentToken rt = tt1.Kit.ProcessReferent("GEO", tt1, null);
                    if (rt != null) 
                    {
                        res.Coef += 0.5F;
                        if (res.Geo == null) 
            ...
            if ((res != null && res.Geo == null && (res.BeginToken.Previous is Pullenti.Ner.TextToken)) && !res.BeginToken.IsValue("УК", null)) 
            {
                Pullenti.Ner.ReferentToken rt = res.Kit.ProcessReferent("GEO", res.BeginToken.Previous, null);
            ...
            if (res != null && res.Geo == null) 
            {
                ...
                {
                    tt2 = tt2.Next;
                    if (tt2 != null && !tt2.IsNewlineBefore && (((tt2.GetReferent() is Pullenti.Ner.Geo.GeoReferent) || (tt2.GetReferent() is Pullenti.Ner.Address.StreetReferent)))) 
                    {
                        res.EndToken = tt2;
                        res.Geo = tt2 as Pullenti.Ner.ReferentToken;
                        if ((tt2.Next != null && tt2.Next.IsAnd && (tt2.Next.Next is Pullenti.Ner.ReferentToken)) && (((tt2.Next.Next.GetReferent() is Pullenti.Ner.Geo.GeoReferent) || (tt2.Next.Next.GetReferent() is Pullenti.Ner.Address.StreetReferent)))) 
                        {
                            tt2 = tt2.Next.Next;
                            res.EndToken = tt2;
                ...
                else if (tt2 != null && (tt2.WhitespacesBeforeCount < 3)) 
                {
                    ...
                    {
                        Pullenti.Ner.ReferentToken rr = tt2.Kit.ProcessReferent("NAMEDENTITY", tt2, null);
            ...
            if (res != null && res.LengthChar == 2 && ((res.Typ == "АО" || res.Typ == "УК"))) 
            {
                ...
                if (tt1 != null) 
                {
                    if (res.Typ == "АО" && tt1.IsValue("УК", null)) 
                        res.IsDoubtRootWord = false;
                    else if (Pullenti.Ner.Core.BracketHelper.CanBeStartOfSequence(tt1, true, false)) 
                        res.IsDoubtRootWord = false;
                    else if (res.Typ == "УК") 
                        return null;
                }
            ...
            if (res != null && res.IsDouterOrg) 
            {
                ...
                {
                    OrgItemTypeToken res1 = _TryAttach(res.BeginToken.Next, true, false);
            ...
            if (res != null) 
            {
                ...
                if (t.IsValue("ГПК", null)) 
                {
                    if (res.Geo != null) 
                        return null;
                    Pullenti.Ner.ReferentToken gg = t.Kit.ProcessReferent("GEO", t.Next, null);
                    if (gg != null || !(t.Next is Pullenti.Ner.TextToken) || t.IsNewlineAfter) 
                        return null;
                    if (t.Next.Chars.IsAllUpper || t.Next.Chars.IsCapitalUpper || Pullenti.Ner.Core.BracketHelper.CanBeStartOfSequence(t.Next, true, false)) 
                    {
                    }
                    else 
                        return null;
            ...
            if (res != null && ((term == "ГК" || term == "ТК" || term == "УК")) && res.BeginToken == res.EndToken) 
            {
                if (res.Geo != null) 
                    return null;
                if ((t.Next is Pullenti.Ner.TextToken) && t.Next.LengthChar == 2 && t.Next.Chars.IsAllUpper) 
                    return null;
                if ((t.Next is Pullenti.Ner.ReferentToken) && (t.Next.GetReferent() is Pullenti.Ner.Geo.GeoReferent)) 
                    return null;
            }
            if (res != null || tt == null) 
                return res;
            ...
            if ((term == "ТС" && t.Chars.IsAllUpper && (t.Previous is Pullenti.Ner.TextToken)) && (t.WhitespacesBeforeCount < 3)) 
            {
                bool ok = false;
                if (t.Previous.IsValue("КОДЕКС", null)) 
                    ok = true;
                else if (t.Previous.LengthChar == 2 && t.Previous.Chars.IsAllUpper && (t.Previous as Pullenti.Ner.TextToken).Term.EndsWith("К")) 
                    ok = true;
                if (ok) 
                {
                    res = new OrgItemTypeToken(t, t) { Typ = "союз", Name = "ТАМОЖЕННЫЙ СОЮЗ", Coef = 4 };
                    return res;
                }
            }
            if (term == "КБ") 
            ...
            if (tt.IsValue("СОВЕТ", "РАДА")) 
            {
                ...
                {
                    Pullenti.Ner.ReferentToken rt = tt.Kit.ProcessReferent("PERSONPROPERTY", tt.Next.Next, null);
                    if (rt != null) 
                    {
            ...
            if (((say || tt.IsValue("ОБЛОЖКА", "ОБКЛАДИНКА") || tt.IsValue("РЕДАКТОР", null)) || tt.IsValue("КОРРЕСПОНДЕНТ", "КОРЕСПОНДЕНТ") || tt.IsValue("ЖУРНАЛИСТ", "ЖУРНАЛІСТ")) || term == "ИНТЕРВЬЮ" || term == "ІНТЕРВЮ") 
            {
                    ...
                    if (ok) 
                    {
                        if ((root == "ИЗДАНИЕ" || root == "ИЗДАТЕЛЬСТВО" || root == "ЖУРНАЛ") || root == "ВИДАННЯ" || root == "ВИДАВНИЦТВО") 
                        {
                            ...
                            if (npt.Adjectives.Count > 0) 
                            {
                                ...
                                {
                                    Pullenti.Ner.ReferentToken rt1 = res.Kit.ProcessReferent("GEO", a.BeginToken, null);
                ...
                Pullenti.Ner.ReferentToken rt = t1.Kit.ProcessReferent("GEO", t1, null);
                if (rt != null && rt.Morph.Class.IsAdjective) 
                {
            ...
            else if ((t.IsValue("ЖУРНАЛ", null) || t.IsValue("ИЗДАНИЕ", null) || t.IsValue("ИЗДАТЕЛЬСТВО", null)) || t.IsValue("ВИДАННЯ", null) || t.IsValue("ВИДАВНИЦТВО", null)) 
            {
                ...
                if (ok) 
                {
                    ...
                    {
                        Pullenti.Ner.ReferentToken rt = t.Kit.ProcessReferent("GEO", t.Previous, null);
            ...
            {
                if (TryAttach(t.Next, canBeFirstLetterLower) != null) 
            ...
            if (t.Morph.Class.IsAdjective && t.Next != null && ((t.Next.Chars.IsAllUpper || t.Next.Chars.IsLastLower))) 
            {
                ...
                {
                    OrgItemTypeToken res1 = _TryAttach(t.Next, true, false);
        ...
        static OrgItemTypeToken TryAttachSpec(Pullenti.Ner.Token t, bool canBeFirstLetterLower)
        {
            if (t == null) 
                return null;
            if (t.Chars.IsLatinLetter) 
                ...
                if (num != null && num.EndToken.Next != null) 
                {
                    ...
                    if (oo != null) 
                    {
                        if (oo.ToString().ToUpper().Contains("МИГРАЦ")) 
                            return new OrgItemTypeToken(t, t) { Typ = (term == "ТП" ? "территориальный пункт" : "миграционный пункт"), Coef = 4, IsDep = true };
                    }
                }
            }
            if (tt.Chars.IsAllUpper && term == "МГТУ") 
            {
                ...
                {
                    OrgItemTypeToken res = new OrgItemTypeToken(tt, tt) { Typ = "главное территориальное управление" };
                    res.AltTyp = "ГТУ";
                    res.Name = "МОСКОВСКОЕ";
                    res.NameIsName = true;
                    res.AltName = "МГТУ";
        ...
        static OrgItemTypeToken _TryAttach(Pullenti.Ner.Token t, bool canBeFirstLetterLower, bool onlyKeywords = false)
        {
            if (t == null) 
                return null;
            ...
            else if ((t is Pullenti.Ner.ReferentToken) && (t.GetReferent() is Pullenti.Ner.Geo.GeoReferent) && (t as Pullenti.Ner.ReferentToken).EndToken.Morph.Class.IsAdjective) 
            {
            }
            else 
                return null;
            ...
            Pullenti.Ner.Core.NounPhraseToken npt = (mc0.IsPronoun ? null : Pullenti.Ner.Core.NounPhraseHelper.TryParse(t, Pullenti.Ner.Core.NounPhraseParseAttr.IgnoreAdjBest, 0, null));
            if (((npt == null && t.Chars.IsCapitalUpper && t.Next != null) && t.Next.IsHiphen && !t.IsWhitespaceAfter) && !t.Next.IsWhitespaceAfter) 
            {
                npt = Pullenti.Ner.Core.NounPhraseHelper.TryParse(t.Next.Next, Pullenti.Ner.Core.NounPhraseParseAttr.IgnoreAdjBest, 0, null);
            ...
            if ((npt == null && (t is Pullenti.Ner.ReferentToken) && (t.GetReferent() is Pullenti.Ner.Geo.GeoReferent)) && (t as Pullenti.Ner.ReferentToken).EndToken.Morph.Class.IsAdjective && (t.WhitespacesAfterCount < 3)) 
            {
                OrgItemTypeToken res1 = _TryAttach(t.Next, true, false);
                if (res1 != null && res1.Root != null && res1.Root.CanBeSingleGeo) 
                {
                    res1.BeginToken = t;
                    res1.Geo = t as Pullenti.Ner.ReferentToken;
                    string nam = Pullenti.Ner.Core.MiscHelper.GetTextValueOfMetaToken(t as Pullenti.Ner.ReferentToken, Pullenti.Ner.Core.GetTextAttr.No);
                    res1.Name = string.Format("{0} {1}", nam, res1.Name ?? res1.Typ.ToUpper());
                    return res1;
                }
            }
            ...
            if (npt == null || npt.InternalNoun != null) 
            {
                ...
                {
                    OrgItemTypeToken res11 = _TryAttach(t.Next, false, false);
            ...
            Pullenti.Ner.Token t0 = npt.BeginToken;
            ...
            foreach (Pullenti.Ner.MetaToken a in npt.Adjectives) 
            {
                Pullenti.Ner.ReferentToken r = npt.Kit.ProcessReferent("GEO", a.BeginToken, null);
                if (r != null) 
                {
                    if (a == npt.Adjectives[0]) 
                    {
                        OrgItemTypeToken res2 = _TryAttach(a.EndToken.Next, true, false);
        ...
        public static bool CheckOrgSpecialWordBefore(Pullenti.Ner.Token t)
        {
            ...
            for (Pullenti.Ner.Token tt = t; tt != null; tt = tt.Previous) 
            {
                ...
                Pullenti.Ner.ReferentToken rt = tt.Kit.ProcessReferent("PERSONPROPERTY", tt, null);
        ...
        public static bool IsTypeAccords(Pullenti.Ner.Org.OrganizationReferent r1, OrgItemTypeToken t2)
        {
            ...
            if (t2.Profiles.Count == 0) 
            {
                ...
                {
                    if (prs.Contains(Pullenti.Ner.Org.OrgProfile.State)) 
                        return false;
                    return true;
                }
            }
        ...
        public static void Initialize()
        {
            ...
            foreach (string s in new string[] {"ФИРМА", "КОМПАНИЯ", "КОРПОРАЦИЯ", "ГОСКОРПОРАЦИЯ", "КОНЦЕРН", "КОНСОРЦИУМ", "ХОЛДИНГ", "МЕДИАХОЛДИНГ", "ТОРГОВЫЙ ДОМ", "ТОРГОВЫЙ ЦЕНТР", "ТОРГОВО РАЗВЛЕКАТЕЛЬНЫЙ ЦЕНТР", "УЧЕБНЫЙ ЦЕНТР", "ИССЛЕДОВАТЕЛЬСКИЙ ЦЕНТР", "КОСМИЧЕСКИЙ ЦЕНТР", "ДЕЛОВОЙ ЦЕНТР", "БИЗНЕС ЦЕНТР", "БИЗНЕС ПАРК", "АУКЦИОННЫЙ ДОМ", "ИЗДАТЕЛЬСТВО", "ИЗДАТЕЛЬСКИЙ ДОМ", "ТОРГОВЫЙ КОМПЛЕКС", "ТОРГОВО РАЗВЛЕКАТЕЛЬНЫЙ КОМПЛЕКС", "АГЕНТСТВО НЕДВИЖИМОСТИ", "ГРУППА КОМПАНИЙ", "МЕДИАГРУППА", "МАГАЗИН", "ТОРГОВЫЙ КОМПЛЕКС", "ГИПЕРМАРКЕТ", "СУПЕРМАРКЕТ", "КАФЕ", "РЕСТОРАН", "БАР", "ТРАКТИР", "ТАВЕРНА", "СТОЛОВАЯ", "АУКЦИОН", "АНАЛИТИЧЕСКИЙ ЦЕНТР", "COMPANY", "CORPORATION"}) 
            {

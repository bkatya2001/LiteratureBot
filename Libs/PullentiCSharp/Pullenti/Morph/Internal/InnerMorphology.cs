    ...
    public class InnerMorphology
    {
        ...
        public List<Pullenti.Morph.MorphToken> Run(string text, bool onlyTokenizing, Pullenti.Morph.MorphLang dlang, bool goodText, ProgressChangedEventHandler progress)
        {
            ...
            for (i = 0; i < twr.Length; i++) 
            {
                ...
                if (term.Length > 2) 
                {
                    if (lang.Equals(Pullenti.Morph.MorphLang.UA)) 
                        pureUkrWords++;
                    else if (lang.Equals(Pullenti.Morph.MorphLang.RU)) 
                        pureRusWords++;
                    else if (lang.Equals(Pullenti.Morph.MorphLang.BY)) 
                        pureByWords++;
                    else if (lang.Equals(Pullenti.Morph.MorphLang.KZ)) 
                        pureKzWords++;
                }
                if (lang.IsRu) 
                    totRusWords++;
            ...
            if (!goodText) 
            {
                for (i = 0; i < (res.Count - 2); i++) 
                {
                    ...
                    if (ui1.IsQuot) 
                    {
                        ...
                        {
                            if (defLang.Equals(Pullenti.Morph.MorphLang.UA) || res[i].Language.IsUa || res[i + 2].Language.IsUa) 
                            {
        ...
        List<Pullenti.Morph.MorphWordForm> ProcessOneWord(string wstr, ref Pullenti.Morph.MorphLang defLang)
        {
            ...
            if (lang.Equals(Pullenti.Morph.MorphLang.EN)) 
                return m_EngineEn.Process(wstr, false);
            if (defLang.Equals(Pullenti.Morph.MorphLang.RU)) 
            {
                if (lang.IsRu) 
                    return m_EngineRu.Process(wstr, false);
            }
            if (lang.Equals(Pullenti.Morph.MorphLang.RU)) 
            {
                defLang = lang;
                return m_EngineRu.Process(wstr, false);
            }
            if (defLang.Equals(Pullenti.Morph.MorphLang.UA)) 
            {
                if (lang.IsUa) 
            ...
            if (defLang.Equals(Pullenti.Morph.MorphLang.BY)) 
            {
                if (lang.IsBy) 
                    return m_EngineBy.Process(wstr, false);
            }
            if (lang.Equals(Pullenti.Morph.MorphLang.BY)) 
            {
                defLang = lang;
                return m_EngineBy.Process(wstr, false);
            }
            if (defLang.Equals(Pullenti.Morph.MorphLang.KZ)) 
            {
                if (lang.IsKz) 
                    return m_EngineKz.Process(wstr, false);
            }
            if (lang.Equals(Pullenti.Morph.MorphLang.KZ)) 
            {
                defLang = lang;
                return m_EngineKz.Process(wstr, false);
            }

...
namespace Pullenti.Morph.Internal
{
    public class MorphEngine
    {
        ...
        long _calcWordsCount(MorphTreeNode tn)
        {
            if (tn.LazyPos > 0) 
            ...
            if (tn.RuleIds != null) 
            {
                foreach (int id in tn.RuleIds) 
                {
                    ...
                    if (rule != null) 
                    {
                        ...
                        {
                            foreach (List<MorphRuleVariant> v in rule.MorphVars) 
                            {
                                res += v.Count;
                            }
                        }
            ...
            if (tn.Nodes != null) 
            {
                foreach (KeyValuePair<short, MorphTreeNode> kp in tn.Nodes) 
                {
        ...
        public List<Pullenti.Morph.MorphWordForm> Process(string word, bool ignoreNoDict = false)
        {
                    ...
                    if (glas) 
                    {
                        foreach (MorphRuleVariantRef mvref in tn0.ReverceVariants) 
                        {
                            ...
                            foreach (Pullenti.Morph.MorphWordForm rr in res) 
                            {
                                ...
                                {
                                    if (rr.Class.Equals(mv.Class) || rr.Class.IsNoun) 
                                    {
                                        ok = true;
                                        break;
                                    }
            ...
            if (mc.Equals(Pullenti.Morph.MorphClass.Verb) && res.Count > 1) 
            {
                foreach (Pullenti.Morph.MorphWordForm r in res) 
                {
                    if (r.UndefCoef > 100 && r.Class.Equals(Pullenti.Morph.MorphClass.Adjective)) 
                        r.UndefCoef = 0;
                }
            }
            if (res.Count == 0) 
        ...
        public List<Pullenti.Morph.MorphWordForm> GetAllWordforms(string word)
        {
            ...
            for (i = 0; i < res.Count; i++) 
            {
                ...
                for (int j = i + 1; j < res.Count; j++) 
                {
                    ...
                    if (wf1.ContainsAttr("инф.", null)) 
                        continue;
                    if ((wf.Class.Equals(wf1.Class) && wf.Gender == wf1.Gender && wf.Number == wf1.Number) && wf.NormalCase == wf1.NormalCase) 
                    {
                        cas |= wf1.Case;
                        res.RemoveAt(j);
                        j--;
            ...
            for (i = 0; i < res.Count; i++) 
            {
                ...
                for (int j = i + 1; j < res.Count; j++) 
                {
                    ...
                    if (wf1.ContainsAttr("инф.", null)) 
                        continue;
                    if ((wf.Class.Equals(wf1.Class) && wf.Case.Equals(wf1.Case) && wf.Number == wf1.Number) && wf.NormalCase == wf1.NormalCase) 
                    {
